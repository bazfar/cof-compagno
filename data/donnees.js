    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 4, nom: "Cri du rassemblement (L, 1x/combat)", effet: "Alliés à portée de voix (7 cases) : 1d6+Mod. de CON PV temporaires et +2 attaque pendant 2 tours",
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 4, nom: "Force herculéenne (passive)", effet: "+1d4 DM bonus permanent aux attaques au contact ; avantage automatique aux tests d'Athlétisme",
            mecanique: { type: "passive", usage: { frequence: "libre" }, cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "special", note: "Repassé en passive (validé avec Thomas) : le +1d4 DM est désormais permanent, mécanisé directement dans la formule de dégâts au contact (cf. Personnage.bonusDegatsForceHerculeenne, câblé aux mêmes 3 sites app.js que bonusDegatsArmeChaos/Dechainement/ArmeEnchantee/AvatarPacte) — le dé de l'arme et ce d4 bonus sont lancés séparément puis fusionnés en un seul résultat par lancerFormule, plus de bouton Lancer manuel. Avantage automatique aux tests d'Athlétisme mécanisé via Personnage.aForceHerculeenne, même mécanisme que Endurance de fer (rang 2, même voie) sur le Test de CON. Mention de la capacité de charge retirée du texte : jamais chiffrée par l'app (pas de notion de charge/poids transporté), hors périmètre." } ] } },
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 2, nom: "Terrain favorable (L)", effet: "Zone 1 case en terrain difficile pendant 3 tours — gênant uniquement pour les ennemis",
          ...
          { rang: 3, nom: "Fortification de fortune (L)", effet: "Barricade improvisée sur 1 case (10 PV + Niveau). Alliés abrités : +4 DEF à distance, +2 au contact",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, cible: "zone", portee: null, zone: 1, jetOppose: null,
          ...
          { rang: 5, nom: "Bastion improvisé (L, 1x/combat)", effet: "Zone 3 cases en bastion pour le reste du combat : alliés +2 DEF en continu ; tout monstre y pénétrant subit une attaque d'opportunité au contact",
  ...
  barde: {
    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          { rang: 1, nom: "Note discordante (sort, L)", effet: "Attaque magique, portée 12 m : -2 à un type de test au choix (attaque, DEF, ou tests de carac.) pendant [3+Mod. de CHA] tours. Non cumulable sur une même cible : une nouvelle Note discordante lancée sur une cible déjà affectée remplace l'effet en cours, elle ne s'y ajoute pas",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, cible: "ennemi", portee: 6, zone: null,
              ...
              effets: [ { type: "bonus", cible: "choix",
                ...
                { type: "special", note: "Mécanisé (validé avec Thomas) : le type de test pénalisé devient un vrai choix à l'activation (attaque, DEF, ou l'une des 6 caracs brutes — même mécanisme que Barde « Premier brassage », Voie de l'alcoolisme), au lieu d'être figé sur 'attaque' par défaut. differe:true ajouté au passage (bugfix, cf. Feinte/Requiem du silence) : le malus ne s'applique désormais que si l'attaque touche. Non-cumul sur une même cible : règle de table, jamais automatisée — la cible typique est un jeton monstre, qui ne suit pas etatsActifs côté app (aucun suivi d'état automatique pour les monstres), donc rien à dédoublonner en code ; à la charge du MJ." } ] } },
          ...
          { rang: 3, nom: "Chant brisant (L)", effet: "Zone 2 cases, portée 7 cases : tous les ennemis subissent -2 attaque et -2 DEF pendant [3+Mod. de CHA] tours. Non cumulable avec Note discordante sur une même cible : le malus le plus fort prime, les deux ne s'additionnent pas",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, cible: "zone", portee: 7, zone: 2, jetOppose: null,
              effets: [ { type: "bonus", cible: "attaque", valeur: -2, duree: "3+Mod.CHA" },
                ...
                { type: "special", note: "Texte mis à jour pour correspondre à mecanique.portee/zone, déjà exprimés en CASES (cf. _armerCiblageCarte côté app.js) — 2 cases de zone à partir de la case cible (validé avec Thomas), 7 cases de portée (15 m). Au lancer, sélection d'un monstre de la zone via un picker armé sur la carte (jetons en portée surlignés), avec aperçu au survol des AUTRES jetons dans le rayon de zone (halo orange, cf. rendreTokensDD/rayonZoneCases côté js/carte.js) — le reste (application du -2 attaque/DEF à TOUS les ennemis de la zone) reste manuel, comme les autres capacités de zone à bonus (ex. Cri du rassemblement). Non-cumul avec Note discordante : même remarque que pour elle — règle de table, à la charge du MJ, aucun suivi d'état automatique côté monstre pour dédoublonner en code." } ] } },
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 2, nom: "Don corrompu (passive)", effet: "Peut lancer n'importe quel sort de sa classe en payant son coût en CS (= rang du sort) au lieu de PP",
            mecanique: { type: "passive", usage: { frequence: "permanente" },
              cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "special", note: "Au moment de l'activation d'un sort, le joueur peut choisir de payer coutCS = rang du sort à la place de coutPP — case à cocher 'Payer en CS' dans le picker de ciblage (cf. app.js), lue par Capacites.lancer via le paramètre payerEnCS." } ] } },

          { rang: 3, nom: "Supplément corrompu (passive)", effet: "Supplée n'importe quel Point de Cercle manquant (Bénédiction/Conviction/Bannissement/Jugement) en payant 2 CS",
            mecanique: { type: "passive", usage: { frequence: "permanente" },
              cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "special", note: "Si le pool de Points de Cercle concerné est insuffisant au moment d'activer un sort qui en dépend, propose de payer 2 CS par point manquant à la place — case à cocher 'Supplément corrompu' dans le picker de ciblage, lue par Capacites.lancer via le paramètre payerSupplementCS." } ] } },

          { rang: 4, nom: "Corruption motivante (activable)", effet: "Prenez X d4 dégâts (X = corruptionMajeure + corruptionCombat actuels) ; gagnez +X à votre prochain jet",
            mecanique: { type: "activable", usage: { frequence: "libre" },
              cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "special", note: "X = (this.corruptionMajeure||0) + (this.corruptionCombat||0). Inflige XD4 dégâts au lanceur lui-même, puis +X au prochain jet (attaque, sauvegarde, ou test — au choix du joueur au moment de l'activation)." } ] } },
    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 5, nom: "Légion de squelettes (1x/jour)", effet: "Invoque des squelettes pendant [niveau] tours, rayon 10 cases : 3d6 DM/tour auto (réduits à 1d6 si action limitée d'opposition)",
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 5, nom: "Moisson d'âmes (L, 1x/scénario)", effet: "[3+Mod. d'INT] tours : capture automatiquement l'âme de toute créature qui meurt dans un rayon de 10 cases, sans test ni limite de stockage",
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 3, nom: "Vrille de réalité (L, zone 2 cases, 7 cases)", effet: "Attaque magique par cible vs DEF : 4d6 DM. +2 CS. 18-20 naturel : jet table Palier 1",
    ...
    voies: [
          ...
          { rang: 2, nom: "Terrain naturel", effet: "Aucune pénalité de déplacement en terrain difficile (neige, boue, broussailles) ; +2 attaque et DEF lors d'un combat dans ces conditions",
            mecanique: { type: "passive", usage: { frequence: "libre" }, cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "bonus", cible: "attaque", valeur: 2, duree: "permanente" }, { type: "bonus", cible: "DEF", valeur: 2, duree: "permanente" },
                { type: "special", note: "Mécanisé : +2 attaque/+2 DEF câblés en permanent dans Personnage.bonusAttaqueCapacites()/bonusDefCapacites() — condition de terrain difficile naturel non trackable dans l'app, simplifiée en bonus permanent dès l'acquisition (à la table d'ignorer hors contexte adapté, même principe que la mutation Reflet trouble). Absence de pénalité de déplacement en terrain difficile reste non modélisée (pas de mécanique de terrain/déplacement chiffré dans l'app)." } ] } },
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 2, nom: "Symbiose protectrice (passive)", effet: "+2 DEF permanent tant qu'il se trouve en milieu naturel",
            mecanique: { type: "passive", usage: { frequence: "libre" }, cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "bonus", cible: "DEF", valeur: 2, duree: "permanente" },
                { type: "special", note: "Mécanisé : +2 DEF câblé en permanent dans Personnage.bonusDefCapacites() — condition de milieu naturel non trackable dans l'app, simplifiée en bonus permanent dès l'acquisition, même principe que Terrain naturel (Voie de la nature rang 2) ci-dessus." } ] } },
          ...
          { rang: 4, nom: "Rempart vivant (L, zone 2 cases, 7 cases)", effet: "Tous les alliés dans la zone : +2 DEF et 1d6 PV temporaires",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, cible: "zone", portee: 7, zone: 2, jetOppose: null,
              effets: [ { type: "bonus", cible: "DEF", valeur: 2, duree: "finCombat" },
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 4, nom: "Fléau rampant (L)", effet: "Aura 1 case pendant [3+Mod. de SAG] tours : 1d4 DM chaotiques à toute créature (alliée comprise) à portée",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, cible: "zone", portee: null, zone: 1, jetOppose: null,
  ...
  enchanteur: {
    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 5, nom: "Domination (sort, L, 1x/scénario)", effet: "Contrôle total d'une cible humanoïde pendant [Mod. de CHA] jours vs DEF ; nouveau test une fois par jour pour résister",
            mecanique: { type: "limitee", usage: { frequence: "1x/scenario" }, cible: "ennemi", portee: null, zone: null,
              jetOppose: { caracAttaquant: "attaqueMagique", caracDefenseur: "defMentale", difficulteFixe: null },
      ...
      },
      {
        ...
        rangs: [
          { rang: 1, nom: "Éclat chaotique (sort)", effet: "Vos illusions infligent 1d6 DM réels vs DEF de la cible (+1 CS par illusion réussie) ; augmente le coût PP de tout sort de famille Illusion de +2",
            mecanique: { type: "activable", corruption: 1, usage: { frequence: "libre" }, cible: "ennemi", portee: null, zone: null,
              jetOppose: { caracAttaquant: "attaqueMagique", caracDefenseur: "defMentale", difficulteFixe: null },
              effets: [ { type: "degats", formule: "1d6", elementaire: "chaos" },
                { type: "special", note: "Reprend telle quelle la mécanique de l'ancien rang 1 'Illusion vivante' (activable + corruption:1, +1 CS approximé en gain fixe par activation plutôt que conditionné à la réussite, même simplification déjà en jeu). caracDefenseur='defMentale' (cf. Capacites.obtenirVolonteCible, ajouté en parallèle sur origin/thomas/contributions — automatiquement repris par le merge). NOUVEAU (prompt_enchanteur_voie_chaos.md) : +2 au coutPP de tout sort de famille Illusion (Voie de l'enchantement rangs 1 'Illusion'/2 'Fascination') tant que ce rang est acquis — malus permanent, pas conditionné à l'utilisation d'Éclat chaotique elle-même, câblé dans Capacites.lancer (coutPPReel), cumulable avec la réduction de Proficience (Voie de l'enchantement rang 4) si les deux voies sont acquises." } ] } },
          { rang: 2, nom: "Drain d'âme (sort)", effet: "La cible subit 1d4+floor(niveau/2) dégâts ; vous récupérez ce montant en PP. Coûte 1 CS",
            mecanique: { type: "activable", usage: { frequence: "libre" }, corruptionCout: 1,
              cible: "ennemi", portee: 10, zone: null,
              jetOppose: { caracAttaquant: "attaqueMagique", caracDefenseur: "defMentale", difficulteFixe: null },
              effets: [ { type: "degats", formule: "1d4+floor(niveau/2)", elementaire: "chaos" },
                { type: "special", note: "Formule calibrée (01/08/2026) pour rester stable à ~22-27% de ppMax sur toute la progression. Le lanceur récupère en PP un montant égal aux dégâts EFFECTIVEMENT infligés (degatsNets, plafonné aux PV réellement retirés à la cible et au ppMax du lanceur) — câblé via Capacites._appliquerGainPPDrainAme, même patron que _appliquerVolDeVie (Nécromancien Voie du sang) mais en PP plutôt qu'en PV. coutCS renommé en corruptionCout côté mecanique (champ réel déjà utilisé par le moteur, cf. Guerrier Rage incontrôlée) — 1 CS. caracDefenseur='defMentale' (cf. Capacites.obtenirVolonteCible, ajouté en parallèle) plutôt que 'DEF' — cible désormais la résistance mentale de la cible, pas son armure." } ] } },
          { rang: 3, nom: "Illusion de masse (L, zone 5 cases)", effet: "Cibles en zone : Sauvegarde Volonté DD 15 ou sombrent dans la folie par les chuchotements et illusions provoquées — attaquent la créature la plus proche pendant 2 tours. Coûte 2 CS",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, corruptionCout: 2,
              cible: "zone", portee: 10, zone: { taille: 5 }, jetOppose: null,
              jetSauvegardeFixe: { carac: "Volonte", dd: 15 },
              effets: [ { type: "etat", id: "folie_illusoire", duree: "2" } ] } },
          { rang: 4, nom: "Murmure terrifiant (L)", effet: "Chuchote dans l'esprit d'une cible : Sauvegarde Volonté DD 16. Échec : jet de 1d20 — 19-20 = la cible se suicide de folie, 16-18 = 1d10 dégâts, en dessous de 16 = folle pendant 3 tours. Coûte 4 CS",
            mecanique: { type: "limitee", usage: { frequence: "libre" }, corruptionCout: 4,
              cible: "ennemi", portee: 10, zone: null, jetOppose: null,
              jetSauvegardeFixe: { carac: "Volonte", dd: 16 },
              effets: [ { type: "special", note: "Cas particulier entièrement résolu dans Capacites.lancer (voie==='Voie du chaos' && rang===4 && classe==='enchanteur'), avant le pipeline générique — la table à 3 paliers post-échec (19-20 mort, 16-18 1d10 DM, <16 état 'folie_illusoire' 3 tours) n'a pas d'équivalent dans mecanique.effets. Non automatisable contre un monstre (le bestiaire n'expose pas ses modificateurs de caractéristique) : message manuel dans ce cas, aucun jet ni effet appliqué automatiquement." } ] } },
          { rang: 5, nom: "Ascension chaotique (L, 1x/scénario)", effet: "Draine tous vos PP et ajoute 5 CS : double votre PV max, double vos bonus de caractéristiques, +4 aux jets d'attaque rapide, double votre pool de PP (immédiatement rempli au nouveau max)",
            mecanique: { type: "limitee", usage: { frequence: "1x/scenario" }, coutPP: "tout", corruptionCout: 5,
              cible: "soi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "etat", id: "ascension_chaotique", duree: "3+Mod.CHA" },
                { type: "special", note: "Durée assumée à [3+Mod.CHA] tours (non précisée explicitement par Thomas), même patron que les autres capstones Voie du chaos Enchanteur/Magicien. Double TOUS les modificateurs de caractéristique (Personnage.mod(), donc CA/attaque/DM/DD des sorts en cascade) et ajoute +4 à bonusAttaqueCapacites (tout type). PP : rempli immédiatement au nouveau ppMax doublé après la pose de l'état (câblé dans Capacites.resoudreEffet, cf. commentaire) — volontairement PAS de second multiplicateur ×2 explicite dans calculerPPMax() en plus du mod() déjà doublé, pour éviter le double-comptage signalé par le design (le pool de PP augmente donc via la cascade du mod doublé, pas d'un strict ×2 du total, approximation assumée). PV max : pvMax est un champ stocké (pas recalculé en direct) — un doublement fiable à l'expiration exigerait une infrastructure de sauvegarde/restauration absente du code, approximé par un gain de PV temporaires égal au pvMax actuel (mécanisme existant, non cumulatif, s'estompe avec l'état sans risque de corrompre les PV réels)." } ] } },
        ],
      },
    ],
    creation: [
    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 5, nom: "Piège du grand gibier (L, 1x/scénario)", effet: "Zone 2 cases : 4d6 DM + immobilisation, contre une cible de grande taille",
    ...
    voies: [
      ...
      },
      {
        ...
        rangs: [
          ...
          { rang: 5, nom: "Illumination du juste (L, 1x/scénario)", effet: "Éclat sacré en zone de 1 case : 3d6 DM (6d6 contre morts-vivants / démons / corrompus), et soigne 1d6 + Mod. de SAG à lui-même et ses alliés dans la zone",
        ...
        rangs: [
          ...
          { rang: 2, nom: "Don corrompu (passive)", effet: "Force n'importe quel sort/capacité : +1d6 DM ou relance un dé raté. +1 CS",
            mecanique: { type: "activable", usage: { frequence: "libre" }, cible: "ennemi", portee: null, zone: null, jetOppose: null,
              effets: [ { type: "degats", formule: "1d6", elementaire: "chaos" },
                { type: "special", note: "Alternative possible : relance un dé de dégâts raté au lieu du bonus +1d6. Coût : +1 CS (jauge de Chaos). Force n'importe quel sort/capacité — dépendance inter-capacités non modélisée automatiquement." } ] } },
          { rang: 3, nom: "Vision interdite (L)", effet: "Zone 2 cases, portée 7 cases : 4d6 DM vs DEF. +2 CS. Sur 18-20 naturel, jet sur la table de mutation Palier 1",
            mecanique: { type: "limitee", corruption: 2, usage: { frequence: "libre" }, cible: "zone", portee: 7, zone: 2,
              jetOppose: { caracAttaquant: "attaqueMagique", caracDefenseur: "DEF", difficulteFixe: null },
              effets: [ { type: "degats", formule: "4d6", elementaire: "chaos" },
...
/* Ordre d'affichage des races.
   ...
   Les gnomes existent dans le lore (éveillés par Thelvaan, cf. section
   « Les races du monde ») mais NE SONT PAS une race jouable. Décision de
   Thomas : ne PAS créer d'entrée dans RACES, ne PAS les ajouter ici, et
   ne pas leur écrire de voie raciale. */
const ORDRE_RACES = ["humain", "elfe", "nain", "demi_elfe", "demi_orc", "demi_gobelin"];

/* ============================================================
   LORE DU MONDE

   Refonte cosmogonique 2.0 (Août 2026) — décisions délibérées, ne pas
   « corriger » sans relire prompt_refonte_cosmogonie.md :
   - Le sous-concept Montagne disparaît de l'arbre des concepts. Les Nains
     ne sont plus la création d'un dieu Gardien de la Montagne : ce sont
     des esprits de la terre abrités par Valdaan pendant la Guerre du
     Trône, éveillés par la protection elle-même.
   - Khoreth n'est plus le miroir de Valdaan (« pierre qui écrase,
     profondeurs »). Elle est désormais le visage de l'Avidité, qui
     dispute le sous-concept Commerce à Kessaan — cf. section
     "Religion & panthéon" et "Notes MJ — la Cour et les Fondations".
   - Les Sylvareth (quatrième nation elfique, déicides d'Ysmaal) sont
     volontairement absents de la section publique "Les races du monde" :
     leur section dédiée ("Les Sylvareth — les elfes de l'archipel") porte
     seule `mjSeulement: true` et concentre tout leur lore. Ne pas
     réintroduire de mention nominative des Sylvareth dans une section
     visible des joueurs.
   ============================================================ */
const LORE = {
  titre: "Lore du Monde — Chroniques Oubliées Fantasy",
  intro:
    "Campagne maison — Document de référence · Version 2.0 (Août 2026). L'Arbre-Monde, la Fracture, la Mer des âmes, les concepts et sous-concepts divins, les races, les nations humaines (Solvarn · Royaumes Coalisés · Liberra), la géographie, la magie, le panthéon et le Chaos.",
  sections: [
    {
      titre: "Cosmogonie et origines du monde",
      categorie: "Fondations du monde",
      contenu:
        "L'Arbre-Monde\n\nÀ l'origine de tout se dresse l'Arbre-Monde — axis de la création, siège des dieux et source de toute vie. Son écorce est le temps, sa sève est la magie, ses racines plongent dans des plans que nul mortel n'a jamais atteints. À son sommet siège le Trône de l'Arbre-Monde, source du pouvoir divin absolu.\n\nConcepts et visages\n\nLe monde ne fonctionne pas par volonté divine mais par cristallisation : un concept se noue depuis l'intensité qu'il concentre, pas depuis le désir d'un dieu de le créer. Un dieu est le visage d'un concept, il ne le possède pas — et un concept peut porter plusieurs visages, qui s'affrontent en permanence pour l'exclusivité. Ce conflit n'est pas un événement passé, c'est un système vivant : il continue de se jouer aujourd'hui, dans chaque prière qui va à l'un plutôt qu'à l'autre. Les sous-concepts voisins connaissent la même dispute, à plus petite échelle — l'Art contre l'Artisanat, la Moisson contre le Fleuve — et pour la même raison : la frontière entre deux domaines proches ne s'est jamais fixée, elle se négocie en continu.\n\nLa Guerre du Trône, dès lors, n'a jamais été une guerre de conquête. C'était une querelle d'exclusivité : chaque visage voulait être le seul titulaire de son concept, et des siècles de sang versé n'ont servi que cette seule ambition, dressée en dogme cosmique.\n\nLe panthéon réel\n\nCinq concepts majeurs se disputent le monde. Deux d'entre eux — la Vie et l'Espace — sont tenus par des visages qui ne se sont jamais mêlés des mortels, trop occupés d'eux-mêmes ou trop lointains pour y songer. Ce point mérite d'être vu sans être souligné : la guerre qui a fracturé le monde fut, pour l'essentiel, une querelle de préséance entre dieux mineurs et moyens, tandis que les vrais majeurs regardaient ailleurs. Les deux visages de l'Espace n'ont jamais été nommés ni vénérés par personne — et ils sont, pour cette raison précise, les seuls à être sortis intacts de la Fracture.\n\nLe déicide d'Ysmaal\n\nYsmaal, quatrième Gardien, visage de la Mort, était le seul concept majeur du panthéon à n'avoir jamais été apparié. C'est précisément pour cette raison qu'il fut choisi : le seul endroit du panthéon où l'on pouvait tenter de dissoudre un concept sans frapper l'un des siens.\n\nLes Enfants de la Sève ont poussé leurs servants elfes à accomplir un rituel destiné à défaire le concept de Mort — pas à tuer un dieu. Mais un dieu est son concept : atteindre l'un revient à frapper l'autre. Deux mille ans de guerre ont été menés sur une erreur métaphysique. On ne peut pas déposséder un dieu de son concept ; il n'y a rien à lui prendre, puisqu'il n'est rien d'autre que cela. Le rituel a fracturé littéralement le continent occidental et séparé l'ancienne capitale elfique du reste du monde — naissance de l'archipel au nord-est. « La Fracture » cesse, à partir de cet instant, d'être une métaphore politique.\n\nCe sont des mortels qui ont tué un dieu. Donc c'est reproductible. C'est le secret le mieux gardé du monde, et presque personne ne sait le lire.\n\nLa Fracture — conséquences\n\n• Défaite des Enfants de la Sève : bannis, et non exécutés — la mort d'Ysmaal a montré à tout le panthéon ce que coûte la disparition d'un concept, et personne n'a voulu réitérer l'expérience sur les vaincus.\n• Bannis dans la Mer des âmes : le seul plan sans gardien, puisqu'ils venaient d'en faire tuer le berger. Ironie centrale de la sentence : le déicide a fabriqué leur propre prison, et cette prison est un garde-manger (cf. section « La Mer des âmes et le trône vide »).\n• L'Arbre blessé : ses racines sont partiellement corrompues. Il dépérit lentement depuis — un déclin que les Elfes ressentent dans leur chair.\n• Naissance des Humains : le sang versé pendant la Fracture, mêlé des deux factions, tombe sur le monde et prend vie. Les Humains portent l'écho de la guerre — double nature instable, susceptible au Chaos.\n• Continent occidental ravagé : l'essentiel des combats s'y est déroulé. Les Elfes qui y vivaient sont décimés ; les survivants se replient sur le continent oriental, où naissent les futures patries d'Aetharion, d'Aelindra et de Mordanel. Seules des enclaves occidentales subsistent, aujourd'hui disputées par les nations humaines (cf. Géographie & capitales).",
    },
    {
      titre: "La Mer des âmes et le trône vide",
      categorie: "Fondations du monde",
      contenu:
        "La Mer des âmes est le cycle de mort et de réincarnation de ce monde. Ysmaal en était le berger — celui qui guidait les âmes mortes vers leur prochaine vie. Depuis sa mort, personne ne tient le poste.\n\nLes trois Enfants de la Sève bannis y sont enfermés, privés de tout ancrage mortel : plus une prière, plus un culte, rien pour les nourrir de la façon dont un dieu se nourrit d'ordinaire. Pour survivre à cette privation, ils se nourrissent de ce qui passe — les âmes.\n\nLes démons sont des restes de repas : des âmes corrompues par le passage d'un dieu affamé, puis façonnées en serviteurs. Ce ne sont pas des créatures neuves fabriquées de toutes pièces — ce sont d'anciens morts, digérés puis reformés.\n\nLes morts-vivants ont une origine distincte : ils viennent du trou laissé par Ysmaal — des âmes que nul ne réclame, faute de berger pour les orienter. Aucun chevauchement avec les démons : deux défaillances du même système, pas une seule.\n\nLe Nécromancien, dès lors, ne sert aucun dieu et n'en tire aucun pouvoir : il pille une vacance. Chaque mort-vivant qui se relève est la preuve ambulante qu'un dieu a été assassiné et que personne n'a repris le service — et le monde vit avec cette preuve sous les yeux depuis des millénaires, sans savoir la lire.\n\nLa réincarnation elle-même est devenue erratique depuis la Fracture, faute de berger pour la régler : les âmes ordinaires errent plus longtemps, se réincarnent de façon moins prévisible, parfois se perdent en chemin.\n\nLes âmes elfiques suivent une autre route : elles ne rejoignent pas la Mer, elles retournent à l'Arbre. Mais la Fracture a percé le canal qui les y ramène, et une part d'entre elles fuit désormais vers la Mer. C'est la vraie raison de la terreur elfique devant le dépérissement de l'Arbre : ce n'est pas de la nostalgie écologique, c'est leur au-delà qui se vide par une fissure.",
    },
    {
      titre: "Les races du monde",
      categorie: "Fondations du monde",
      contenu:
        "Les Elfes — Enfants de l'Arbre\n\nFaçonnés par les dieux à partir de la sève de l'Arbre-Monde : enfants de l'Arbre par la matière, servants des dieux par la fonction. Ils servaient les visages avant le schisme ; la Guerre du Trône les a scindés comme elle a scindé le panthéon. Sang de sève pure → résistance au Chaos (+2 ou +rang aux jets contre la corruption de la Voie du Chaos). Ils ressentent physiquement la blessure de l'Arbre. Trois nations :\n\n• Aetharion (Hauts Elfes) : préserver et isoler l'Arbre. Hautains. Bannissent les demi-elfes. Mépris pour les Aelindra, méfiance pour les Mordanel.\n• Aelindra (Elfes Sylvains) : réparer l'Arbre en tissant des liens avec toutes les races. Présents dans la République. Acceptent les demi-elfes.\n• Mordanel (Elfes du Crépuscule) : garder la mémoire et témoigner. Fracture interne : Anciens prudents contre Jeunes qui veulent agir. Accusés par l'Empire d'avoir provoqué l'incarnation de la Traque.\n\nLes Humains — Sang Mêlé, Fracturé\n\nNés du sang divin des deux camps. Ambition insatiable, adaptabilité. -2 aux jets contre la corruption du Chaos, mais accès plus facile à la Voie du Chaos (les consciences les reconnaissent).\n\nLes Nains — Esprits de la Terre Réfugiés\n\nEsprits de la terre que Valdaan a abrités en son sein pendant la Guerre du Trône ; c'est la protection elle-même, prolongée par des siècles de conflit, qui les a éveillés à la conscience. Les Nains ne sont donc pas la créature d'un artisan divin : ce sont des réfugiés de guerre devenus peuple. Ancrage dans l'Ordre.\n• Nains de l'Ordre : forge, tradition, alliés de l'Empire (acier et cols fortifiés). +1 contre la corruption.\n• Khazrak Dûm (Nains Renégats) : après le Scellement des Portes du Sud (cf. Histoire — Le Scellement des Portes du Sud), qui emmure leurs garnisons hors de Kaldrun pour sauver la capitale, ces tribus sont livrées à elles-mêmes face aux tribus orques et gobelines des Failles Rouges. Des générations de siège sans espoir de renfort les poussent finalement à un pacte de survie avec leurs assaillants plutôt qu'à leur destruction — pacte qui les transforme peu à peu : plus grands, peau grisée, mâchoire proéminente. En dominant ce qu'ils méprisaient, ils sont devenus ce qu'ils haïssaient. Le Scellement a aussi détruit leur chaîne de commandement : organisés en clans indépendants plutôt qu'en pouvoir central, scindés entre Résistants (refusent toute assimilation nouvelle, espèrent encore une réconciliation avec Kaldrun) et Évolutionnistes (ont intégré le pacte comme identité, enviant en secret la pureté rituelle jamais compromise des Nains de l'Ordre — un ressentiment devenu, de génération en génération, un rêve de reconquête de Kaldrun). Observés par les Dieux du Chaos.\n\nLes Gnomes — Esprits de la Forêt, Éveil de Seconde Main\n\nEsprits de la forêt abrités par Thelvaan, dieu de la Forêt, qui a imité le geste de Valdaan envers les Nains pendant la même guerre. Les Gnomes sont donc un éveil de seconde main — et leur goût pour la forge n'est pas une convergence mystérieuse : c'est l'imitation qui leur a transmis le geste avec le reste. Physiquement moins barbus que les Nains, moins portés sur la boisson, nettement plus loquaces et exubérants. Affinité marquée pour la magie classique, là où les Nains passent par la Pierre. Considérés comme des demi-hommes par les nations humaines, autant méprisés que respectés pour leur savoir. Vivent surtout dans les forêts elfiques, en cohabitation paisible avec les Aelindra — mais les elfes les voient en cousins à protéger, pas en égaux, et cette condescendance a un précédent divin : même leur naissance est une copie.\n\nOrcs & Gobelins — Émotions Cristallisées\n\nNés des émotions de violence de la Guerre du Trône, ils créèrent des corps pour traquer et tuer les races de l'Ordre.\n• Orcs — corps du Meurtre (meurtre glorieux) : combattent en horde, face à l'ennemi, vous regardent dans les yeux avant de tuer.\n• Gobelins — corps de la Traque (traque prédatrice) : embuscades, pièges, nombre. Tuent par instinct. Plus malléables — certaines tribus servent les Khazrak Dûm.",
    },
    {
      titre: "Le Chaos — hiérarchie et manifestations",
      categorie: "Fondations du monde",
      contenu:
        "Hiérarchie du Chaos\n\n• Niveau I — Dieux du Chaos : Enfants de la Sève bannis, pleinement cristallisés. Absents physiquement, influence indirecte et plans à long terme.\n• Niveau II — Consciences-émotions (Violence / Meurtre / Traque) : pas encore des dieux, millénaires d'existence. Bénédictions sur champions ; rarement, incarnation temporaire (siècles de recharge). Ce n'est pas un étage décoratif de la hiérarchie : ce sont des concepts en cours de cristallisation, que les Enfants déchus cultivent activement pour se reconstituer un ancrage. C'est le vrai moteur du Chaos.\n• Niveau III — Démons : des âmes corrompues par le passage d'un dieu affamé dans la Mer des âmes, puis façonnées en serviteurs — pas des créatures neuves fabriquées par les Dieux du Chaos (cf. section « La Mer des âmes et le trône vide »). Actifs dans le monde, outils de destruction de l'Ordre.\n• Niveau IV — Orcs & Gobelins : corps des consciences dans le monde mortel. Omniprésents, instruments sans dévotion consciente.\n\nL'Incarnation de la Traque — le Silence de Valmoire (il y a ~18 ans)\n\nPour la première fois depuis des siècles, la conscience de la Traque s'est incarnée, dans une zone frontalière disputée entre les terres Mordanel et les marches orientales de Solvarn. Le village de Valmoire a disparu en trois semaines — les habitants traqués un par un, retrouvés des jours après sans marque de lutte. Cette méthode silencieuse et patiente a permis à l'Empire de désigner les Elfes du Crépuscule : \"Qui d'autre traque ainsi, sinon un peuple qui vit dans l'entre-deux du jour et de la nuit ?\"\n\nL'Empire a exploité l'événement pour élargir son accusation à Aetharion également — cible plus \"légitime\" aux yeux de l'opinion solvarienne (riche, isolationniste, déjà mal-aimée) qu'un peuple pauvre et dispersé. Mensonge délibéré ou conviction sincère ? Ambiguïté narrative centrale de la campagne. Et si les Dieux du Chaos avaient manipulé les deux camps simultanément ?\n\nChez les Mordanel, l'événement est encore frais : la fracture interne entre Anciens (retenue) et Jeunes (réponse armée) n'est pas tranchée. Les dryades proches des terres Mordanel se sont en partie retirées depuis le Silence — un désaveu silencieux qui aggrave la fracture (voir section Les Dryades).\n\nTrois tentatives, une seule ignorance\n\nLa quête de Valdorne pour l'Aspect de la Bravoure et l'Église de Solvarn emploient, sans le savoir, la même méthode que les Enfants déchus : cultiver une émotion jusqu'à ce qu'elle cristallise en ancrage. Trois tentatives parallèles de fabriquer un ancrage divin ; celle qui réussit le mieux est précisément la seule qui ignore qu'elle le fait.",
    },
    {
    ...
    },
    {
      ...
      categorie: "Fondations du monde",
      contenu:
        "Deux sources, pas une. La ligne canonique d'autrefois — une source unique, trois prismes d'accès — ne tenait pas : les Humains n'ont jamais eu d'accès natif à l'Arbre-Monde.\n\nSource 1 — L'Arbre-Monde\n\nLa Sève (magie elfique · intuitive · organique)\nInstinctive pour les Elfes (se souvenir, pas apprendre). Liée à la vie et aux cycles ; se raréfie avec le déclin de l'Arbre. Humains/Nains peuvent l'apprendre, avec effort. COF : +2 aux jets de magie innée pour les Elfes.\n\nLa Pierre (magie naine · lente · permanente)\nRunes, forge, enchantements durables — les Nains parlent de travail, pas de magie. Lente mais ineffaçable. Ancre dans l'Ordre. COF : +1 aux jets contre la corruption pour les Nains de l'Ordre.\n\nLes Runes de sève (magie gnome · méthode transmissible)\nLes Gnomes ont appliqué la méthode naine à la sève elfique. C'est la seule magie enseignable du monde : une méthode transmissible posée sur une source vivante. D'où le respect que leur portent les nations humaines pour leur savoir, et leur place naturelle aux Académies de Liberra. D'où aussi le malaise elfique — un gnome fait avec des outils ce qu'un elfe fait de mémoire. Second acte d'imitation d'un peuple qui n'existe que par imitation : Thelvaan a imité Valdaan pour les éveiller, eux ont imité les Nains pour accéder à la magie.\n\nLe flux de l'Arbre diminue depuis la Fracture. Elfes et Gnomes tirent donc sur le même réservoir déclinant ; la méthode gnome les rend seulement plus efficaces sur une réserve qui se vide.\n\nSource 2 — La Mer des âmes (magie humaine)\n\nLes Humains n'ont pas été faits de l'Arbre : ils n'y ont aucun accès natif. Ils ont donc puisé dans le seul autre réservoir — la Mer des âmes.\n\nCette magie ne fonctionne que parce qu'Ysmaal est mort. La Mer est accessible précisément parce qu'elle n'a plus de berger. L'Église de Solvarn, qui a le plus intérêt à ce que nul ne se souvienne du quatrième Gardien, tire toute sa puissance du crime qu'elle efface.\n\nL'ancien prisme portait le nom de « Sang divin » : le nom est trompeur, les Humains ne puisent pas dans leurs veines mais dehors. C'est pourtant le nom que l'Église emploie encore, et elle y tient — reconnaître la source reviendrait à reconnaître qu'il fallait un dieu mort pour l'ouvrir.\n\nÇa explique l'ascendant humain : le déclin de l'Arbre n'affecte pas leur source. Les Elfes ne déclinent pas parce que les Humains montent — les deux courbes n'ont rien à voir, et les Elfes ne l'ont jamais compris.\n\nLa Voie du Chaos n'est plus un système à part. Ce n'est plus « la même source par les racines corrompues » : c'est le fond de la Mer, la partie où les trois bannis chassent. Les paliers de corruption deviennent une mesure de profondeur :\n• L'Effleurement (0-2 échecs) : yeux qui virent, voix dédoublée, plantes qui flétrissent. Aucun effet mécanique — les autres remarquent.\n• La Marque (3-5 échecs) : trait physique permanent (veine noire, mèche décolorée). -1 SAG ou CHA ; les Aetharion refusent de parler.\n• La Fracture (6+ échecs) : visions de la Guerre du Trône, voix des consciences. +1 dé de dégâts en Voie du Chaos mais jets avec désavantage.\nLe -2 humain aux jets de corruption s'explique de lui-même : ils nagent déjà dedans.\n\nConseil MJ : ne pas tracker les échecs devant les joueurs — révéler les paliers narrativement.\n\nLe filtre divin\n\nTout dieu suffisamment ancré peut filtrer la Mer pour ses fidèles — mais filtrer draine sa puissance. Conséquence sociale : la magie sûre est un privilège de religion riche. Seul un culte massif peut se payer un filtre ; les cultes marginaux et les sous-concepts à fidèles clairsemés ne filtrent pas, leurs pratiquants lancent à nu.\n\nAethar filtre pour l'Église de Solvarn. La doctrine impériale — « magie réservée aux agréés, le reste est hérésie » — est donc matériellement exacte. L'Église a raison, pour des raisons qu'elle a en partie oubliées et en partie maquillées.\n\nBoucle perverse : le culte solvarien nourrit Aethar, le dépense en filtrant, et le déforme en le priant. Un dieu devenu dogmatique filtre plus étroitement — ce que les fidèles lisent comme « le Soleil accorde moins aux indignes », alors que c'est l'inverse : il n'en a plus les moyens.\n\nLes trois bannis ne peuvent filtrer pour personne — aucun ancrage. Quand Khoreth offre à un hôte un accès direct, sans clergé, sans autorisation, elle ne fait pas un cadeau : elle maquille en générosité la seule chose qu'elle ait à donner. C'est l'argumentaire d'un usurier, ce qui lui va bien.",
    },
    {
      titre: "Le Vieil Aetharien — langue morte des érudits",
      categorie: "Fondations du monde",
      contenu:
        "Avant que Solvarn ne réinterprète Aethar en Soleil-Dieu unique, le culte du Premier Gardien — Lumière & Temps — disposait déjà d'une langue liturgique propre, portée par ses érudits et prêtres-astronomes bien avant la fondation de l'Empire. Le Vieil Aetharien a survécu à la rupture doctrinale qui l'a suivi : plus personne ne le parle aujourd'hui, mais il reste la langue de référence de tout traité savant sérieux à travers le monde connu — alchimie, astronomie, une partie de la théologie comparée — un peu à la manière d'une langue classique que plus aucune nation ne revendique, mais que tous les érudits reconnaissent.\n\nParticularité du système numéral (souvent négligée par les érudits pressés) : les nombres du Vieil Aetharien n'ont jamais été standardisés — chaque école de prêtres-astronomes du culte primitif comptait selon sa propre tradition régionale, certaines en base dix, d'autres en base vingt, quelques-unes selon des systèmes mixtes propres à telle ou telle province d'Aetharion. Un lexique de vocabulaire général traduit sans difficulté noms, verbes et concepts ; les nombres, eux, résistent à toute traduction mécanique et exigent un vrai travail d'érudition (Histoire) pour être interprétés correctement selon leur contexte d'origine.\n\nLa langue survit surtout aujourd'hui dans des fragments de traités alchimiques anciens, conservés dans certaines bibliothèques familiales ou monastiques — vestiges d'une érudition que plus d'une famille de marchands a fini par troquer contre le commerce, sans toujours s'en souvenir.",
    },
    {
      titre: "Religion & panthéon",
      categorie: "Fondations du monde",
      contenu:
        "Le décompte des mortels est un décompte de survivants. Les peuples connaissent six dieux ; il y en eut sept majeurs impliqués dans la Guerre du Trône, dont Ysmaal. Presque personne ne sait plus qu'un quatrième Gardien a existé — et l'Église de Solvarn a un intérêt évident à ce que nul ne s'en souvienne : un dieu mort, c'est la preuve qu'un dieu peut mourir.\n\nLes Académies de Liberra sont le seul endroit du monde où l'on sait encore compter les vrais dieux, et où les noms d'Ithanor et de Sethrin, visages de l'Intelligence, sont connus. Une école laïque, dans une république sans religion d'État, pendant que l'Empire fait prier des millions de gens un soleil qu'il a inventé.\n\nAethar est en train de devenir le mensonge qu'on prie. Le culte solvarien le nourrit massivement — et le déforme à son image : dogmatique, exclusif, intolérant. Les Aetharion, eux, vénèrent encore le Premier Gardien tel qu'il fut : ils prient quelqu'un qui n'existe plus sous cette forme, et personne ne le leur a dit.\n\nValdaan ne grandit pas, il est porté. Ce que les mortels appellent « Ancêtres & Forge » n'est pas une annexion du domaine funéraire : celui-ci reste réellement orphelin depuis la mort d'Ysmaal, et c'est la seule fidélité naine qui projette cette part sur Valdaan. Lui-même la prend pour de la croissance. Conséquence structurelle : si Kaldrun vacille, Valdaan rétrécit avec elle — ce qui donne un poids neuf à la fracture entre Nains de l'Ordre et Khazrak Dûm.\n\nNyaleth, visage déchu de la Vie (Maladie & Putréfaction), est tombé après la Fracture : il n'y avait plus personne d'assez fort, dans un panthéon déjà exsangue, pour le bannir. C'est le seul dieu tombé encore libre et ancré dans le monde — le seul qu'on puisse rencontrer, atteindre, marchander ou tuer. La peste n'est pas un fléau naturel : c'est une présence.\n\nKhoreth quitte la branche naine : elle est le visage de l'Avidité, qui dispute le Commerce à Kessaan — celle qui aime l'objet et non le geste qui le produit. Le Comptoir de Libris vénère Kessaan sans toujours y penser.\n\nReligions par système\n• Église de Solvarn : monothéisme solaire dogmatique. Aethar réinterprété en Soleil-Dieu ; les autres dieux = « démons elfiques ». Magie réservée aux agréés ; le reste est hérésie. Légitime le sang impérial.\n• Cultes des Royaumes : Valdorne (Gardiens sincères), Arveth (culte hybride sous pression solvarienne — le vrai terrain de la rupture), Mornac (foi de façade), Serval (culte de Valdaan hérité des Nains).\n• Liberra : liberté de culte totale, magie libre réglementée. Tension mages vs laïcs craignant le Chaos.\n• Elfes : contact direct, pas de clergé — la magie est mémoire. Nains : la forge est une prière à Valdaan.\n\nLa fracture religieuse d'Arveth : le culte solvarien s'infiltre dans ses temples. Si Arveth bascule religieusement avant de basculer militairement, la coalition perd son ciment moral.",
    },
    {
      titre: "Géographie & capitales",
      categorie: "Fondations du monde",
      contenu:
        "Un continent occidental et un continent oriental, séparés par une mer intérieure — la Mer de Cendre-Claire (les Solvariens) / Vaelthys, \"les eaux qui séparent\" (les Hauts Elfes).\n\nCONTINENT OCCIDENTAL\n• Empire de Solvarn — nord-centre — capitale Solmaris — villes : Fort Soleil (forteresse, nord), Kor Valdan (côte du Golfe d'Acier), Naros (fort-frontière face à Arveth), Durnholk — frontière sud avec Arveth, marches orientales disputées avec l'enclave mordanelle, côte orientale sur la Mer de Cendre-Claire (tête de pont vers Aetharion).\n• Valdorne — ouest — capitale Valdecourt — villes : Gardesèle (côte nord-ouest), Haldren — berceau de la Rupture.\n• Arveth — frontière directe avec Solvarn (le vacillant) — capitale Arvenfall — ville : Boigris (frontière des Marches Orientales).\n• Mornac — traversé par la Lisdane, côte sud — capitale Mornhaven — port secondaire : Port-Saphir — commerce.\n• Serval — Contreforts de Serval, allié aux Nains de l'Ordre — capitale Serval — villes : Grimval, Roc-Épine (grande ville côtière face au Détroit des Brumes) ; cols : Col du Corbeau, Col des Vents.\n• République de Liberra — sud, à l'embouchure de la Lisdane — capitale Libris — port principal : Port-Libris (Grand Port).\n• Enclave occidentale d'Aelindra — côte centre-ouest, frontière naturelle avec Liberra — Bois des Dryades. Communauté sylvaine installée loin de la patrie orientale, sans capitale propre, alliée naturelle de Liberra.\n• Enclave occidentale de Mordanel — marches orientales de Solvarn, côte, site du Silence de Valmoire — tête de pont impériale disputée vers Aetharion ; Fort de l'Aube en marque la frontière. Territoire séparé de la patrie orientale, sans capitale propre.\n• Nains de l'Ordre — Contreforts de Serval — capitale souterraine Kaldrun (\"la Cité sous la Montagne\") — site : Forge-Runez.\n• Khazrak Dûm (Nains Renégats) — les Failles Rouges, à l'est de Liberra, en bordure des terres orques — capitale Karag Dûm — ville : Grimgal. Scindés entre Résistants (proches de Kaldrun) et Évolutionnistes (enfoncés dans les Failles, au contact direct des tribus orques).\n• Tribus Orcs/Gobelins — terres sauvages à l'est de Liberra et au-delà des Failles Rouges.\n\nLA MER DE CENDRE-CLAIRE (VAELTHYS)\nSépare les deux continents. La guerre Solvarn/Aetharion s'y joue par débarquements et sièges côtiers plutôt que par une ligne de front terrestre — une guerre d'usure, sans percée décisive depuis 3 ans.\n\nCONTINENT ORIENTAL\n• Aetharion — côte occidentale face à Solvarn — forêt fermée — capitale Elyndoril — villes : Santhari (côte, face au Détroit des Brumes), Lorienn (grande ville côtière nord-est) — l'Arbre-Monde en son cœur : un lieu réel, caché, connu des seuls Hauts Elfes de haut rang et protégé par un réseau de bosquets à dryades. Officiellement nié comme lieu physique par la doctrine impériale (hérésie païenne) ; en pratique, objet d'enquêtes discrètes des Chevaliers-Inquisiteurs.\n• Aelindra (patrie) — sud d'Aetharion — capitale Cœuvre — villes : Aranil, Valmeryl. Patrie ancestrale des Elfes Sylvains ; l'enclave occidentale (côte de Liberra) en est une communauté séparée, sans lien de gouvernance directe.\n• Mordanel (patrie) — sud d'Aelindra — capitale Valdourt — villes : Myral, Ombreval. Patrie ancestrale des Elfes du Crépuscule ; l'enclave occidentale (marches de Solvarn, Silence de Valmoire) en est un territoire séparé, sans lien de gouvernance directe.\n• Tribus Orcs/Gobelins — marges orientales du continent, au-delà d'Aetharion.\n\nFLEUVES\n• La Sombre — frontière historique Solvarn/Arveth, gué de Fossessainte.\n• La Verselande — cœur des terres coalisées, entre Mornhaven et Valdorne, Pont-Rompu.\n• La Lisdane — source dans les Contreforts de Serval, traverse Mornac, se jette dans la mer près de Libris (alimente le Quartier des Marchés).\n\nL'ARCHIPEL DE LA FRACTURE\nAu large des côtes occidentales, un arc de débris rocheux s'étire du nord-est de l'ancien continent occidental jusqu'à ses franges les plus à l'ouest — vestige de la Guerre du Trône, jamais cartographié comme un tout. Deux tronçons seulement portent un nom chez les humains :\n• Les Ruines Blanches — au large de Solvarn — ruines manifestement elfiques que l'Empire n'a jamais explorées.\n• Les Brisants — plus au sud-ouest, face à Mornac et à Liberra — où l'on extrait depuis peu un minerai ordinaire mais abondant (cf. section « Les Brisants — la ruée »).\nLes érudits des Académies de Liberra donnent à l'ancienne capitale elfique engloutie au cœur de cet arc le nom d'Aethanor — un savoir de bibliothèque, jamais un lieu que quiconque ait revisité.",
    },
    {
      titre: "Des tribus humaines au Premier Empire",
      categorie: "Histoire humaine commune",
      contenu:
    ...
    },
    {
      ...
      categorie: "Histoire humaine commune",
      contenu:
        "Quand la dynastie régnante du Premier Empire a durci sa doctrine solaire, une branche a revendiqué l'exclusivité du sang divin des Gardiens et verrouillé le pouvoir — devenant Solvarn. Quatre grandes maisons provinciales (futurs Valdorne, Arveth, Mornac, Serval) ont refusé cette purification et se sont soulevées ensemble : la Rupture. Guerre de dix ans, achevée sur un armistice que Solvarn n'a jamais considéré comme définitif.\n\nCinq batailles marquantes :\n\n• La Charge de Fossessainte (an 1) — bataille d'ouverture à un gué frontalier. Sire Alaric de Valdorne, jeune chevalier sans grade, rallie les colonnes dispersées et transforme une déroute en victoire. Fondateur du code chevaleresque de Valdorne : l'honneur individuel qui sauve la cause commune.\n\n• La Campagne des Marches (an 2-3) — le commandement solvarien ne s'en prend pas à un maillon faible, mais au contraire au noyau militaire de la coalition : l'infanterie d'Arveth, héritière directe des légions qui tenaient déjà la marche orientale du Premier Empire, est de loin la force de combat la plus redoutable dont disposent les rebelles. Affaiblir durablement ce noyau vaut mieux, calcule l'Empire, qu'une victoire rapide ailleurs — deux ans d'une guerre d'usure d'une brutalité rare, sans manœuvre ni ruse, seulement des lignes qui s'affrontent jusqu'à l'épuisement sur le sol même d'Arveth. Le Général Wyland d'Arvenfall, ancêtre direct de l'actuel Lord Ranulf, tient la ligne en opposant aux légions impériales une armée à l'image de celles qu'Arveth avait elle-même formées — mêmes cohortes, même discipline, jusqu'aux mêmes manœuvres (cf. section suivante sur les légions impériales). Les renforts de cavalerie envoyés par les ordres chevaleresques de Valdorne, encore portés par l'élan de Fossessainte, empêchent l'effondrement complet du front oriental — la première preuve concrète que la rébellion est une coalition réelle, et non quatre soulèvements séparés qui auraient pu tomber un par un. Arveth tient, mais au prix d'un saignement dont le royaume ne se relève jamais complètement : l'origine directe de la vulnérabilité permanente qu'il porte encore aujourd'hui, cent soixante-dix ans plus tard. L'ironie qui perdure : Arveth reste malgré tout le fer de lance de l'infanterie coalisée, quand la cavalerie demeure l'apanage des ordres chevaleresques de Valdorne — une répartition du travail martial héritée directement de cette campagne. C'est de Wyland, dit-on, que vient la devise qu'Arveth porte encore aujourd'hui : « Nous tenons la ligne. »\n\n• Le Siège de Mornhaven (an 4-5) — le Haut-Maréchal Corvain Ashe assiège la capitale de Mornac huit mois. Mornac tient grâce aux convois de Serval à travers ses cols — ce qui soude la Coalition en alliance réelle.\n\n• Le Siège du Col des Vents (an 6-7) — Solvarn ouvre un second front en tentant de forcer les passes de Serval, présumant le royaume montagnard isolé et sous-défendu après avoir vidé ses réserves pour les convois de Mornhaven. L'Empire y trouve à la place le premier déploiement conjoint des forces servalines et des guerriers de l'Ordre des Nains, épaulés par des armes runiques forgées pour l'occasion par le clan Pierrefonde — Margrave Gunnar Sten et le maître d'armes naine Borin Pierrefonde y repoussent l'assaut avec des pertes impériales sévères. Ce n'est plus seulement une alliance diplomatique : c'est la première fois que Serval et les Nains de l'Ordre versent le sang ensemble, l'origine véritable du partenariat métallurgique permanent qui définit encore Serval aujourd'hui.\n\n• La Bataille du Pont-Rompu (an 9) — Alaric, devenu commandant respecté, meurt en détruisant un pont pour couvrir la retraite coalisée. Un mois plus tard, les deux camps épuisés négocient.\n\nL'Armistice des Quatre Sceaux fut négocié et signé un mois après Pont-Rompu, à même les ruines du pont détruit — un choix délibéré des quatre maisons rebelles, qui voulaient que l'Empire signe la paix là où l'un des siens était mort pour l'arracher. Un pavillon de fortune fut dressé sur la rive, quatre tables distinctes disposées en cercle plutôt qu'alignées, pour qu'aucune signature ne puisse jamais sembler placée au-dessus d'une autre.\n\nCôté impérial, l'Empereur Aurelian II signa personnellement, entouré d'une cour visiblement divisée : le Haut-Maréchal Corvain Ashe, dont les deux campagnes n'avaient rien emporté de décisif, se tenait à son épaule, réduit au silence par le protocole mais non par la conviction.\n\nCôté rebelle, chaque maison insista pour apposer son sceau séparément plutôt que d'accepter un sceau commun de la Coalition — non par défiance mutuelle sur le champ de bataille, où l'alliance avait tenu, mais parce qu'aucune des quatre ne voulait que sa souveraineté fraîchement gagnée soit diluée dans une entité collective qui pourrait un jour prétendre la primer. Ce geste, plus que toute clause du traité, a fixé pour cent soixante-dix ans la nature de la Coalition : une alliance de circonstance entre quatre couronnes fières et séparées, jamais une fédération.\n\nLes quatre signataires : Sire Wyland d'Arvenfall pour Arveth, dont la ligne tenue à la Campagne des Marches lui valait autant d'autorité morale que sa couronne ; le Roi Baldwin Ier pour Valdorne, qui porta seul le poids d'un traité qu'Alaric, mort un mois plus tôt, n'aurait peut-être jamais accepté de signer ; la Comtesse Elowen Maren pour Mornac, qui négocia clause par clause avec une minutie que les autres délégations trouvèrent presque indécente, mais qui garantit à son royaume des termes commerciaux qu'aucune autre maison n'aurait pensé à réclamer ; et le Margrave Gunnar Sten pour Serval, venu accompagné d'un maître d'armes nain du clan Pierrefonde — un geste qui scandalisa la délégation impériale et que Serval revendique encore aujourd'hui comme son premier acte de souveraineté retrouvée.\n\nLe traité original, avec ses quatre sceaux distincts jamais réunis sur un seul document commun, est aujourd'hui dispersé : chaque maison conserve sa propre copie scellée, et aucune des quatre n'a jamais accepté de la confier à un dépôt commun — même la paix, chez les Royaumes Coalisés, reste une affaire que l'on préfère garder chacun chez soi.\n\nCorvain Ashe, le Haut-Maréchal qui a mené les deux campagnes les plus coûteuses de la guerre — les Marches puis Mornhaven — sans jamais emporter la victoire décisive qu'il cherchait, tient une place particulière dans la mémoire des deux camps. Pour l'Empire, il reste un commandant capable et dévoué, brisé non par une faute tactique mais par quelque chose de plus profond qu'il a passé le reste de sa vie à nommer : la conviction croissante, presque obsessionnelle, que Solvarn n'a pas perdu par manque de discipline militaire, mais par manque de pureté — des officiers trop cléments, des soldats qui fraternisaient avec des aumôniers ennemis, une foi diluée par des décennies de contact avec des terres qu'on aurait dû purifier plus tôt. Ses écrits, largement ignorés de son vivant, ont été redécouverts par le Sacerdoce Solaire des décennies plus tard — la matière première intellectuelle du durcissement doctrinal qui a donné naissance aux Chevaliers-Inquisiteurs. Corvain Ashe n'a jamais vu naître l'ordre qu'il a, sans le savoir, fondé dans le désespoir d'une défaite qu'il n'a jamais su accepter autrement.\n\nPour la Coalition, en particulier Arveth et Mornac, son nom évoque un tout autre souvenir : celui d'un général qui a bien failli les briser, deux fois, et qui n'a échoué ni par excès de retenue ni par manque de talent — seulement parce qu'un peuple qu'il avait lui-même formé, et un royaume montagnard qu'il avait sous-estimé, ont tenu un peu plus longtemps que lui. Théodren Ashe, son arrière-petit-fils, porte aujourd'hui sa doctrine comme un dogme figé — sans jamais sembler porter le doute et l'angoisse sincères qui l'ont fait naître chez son aïeul.\n\n\"Être un Alaric\", à Valdorne, désigne aujourd'hui un acte de courage qui dépasse son rang.",
    },
    {
    ...
    {
      titre: "Le Grisfer — le seul métal qui compte",
      categorie: "Approfondissements par faction & peuple",
      contenu:
        "Sous Kaldrun s'ouvre une veine unique de métal gris terne, sans éclat, que rien ne distingue à l'œil d'un minerai ordinaire — sinon qu'il frappe plus fort ce qui n'a ni sang ni peur de la mort. Personne n'a jamais su dire pourquoi cette veine-là, et nulle autre, produit cet effet. Ce n'est pas un secret gardé : c'est un accident géologique dont la cosmogonie ne rend aucun compte, et les érudits qui s'y sont penchés en sont ressortis bredouilles.\n\nLes Nains de l'Ordre en détiennent le monopole mondial, alliés qu'ils sont de l'Empire. Le seul contre-poison connu aux démons et aux morts-vivants ne passe donc, pour l'essentiel du monde, que par une seule ville et un seul peuple — Liberra et les Royaumes Coalisés en dépendent entièrement, sans autre source à qui s'adresser.\n\nLes Khazrak Dûm, emmurés hors de Kaldrun depuis le Scellement des Portes du Sud, n'y ont aucun accès. Leur rêve de reconquérir un jour la capitale ne tient donc plus seulement à l'orgueil rituel ou à la mémoire des runes perdues (cf. « Les Nains de l'Ordre — forge et foi ») : sous la montagne dort aussi la seule arme qui compte vraiment contre ce que la Mer des âmes déverse — et ils en sont exclus par le même mur qui les a exclus de tout le reste.",
    },
    {
    ...
    },
    {
      ...
      categorie: "Approfondissements par faction & peuple",
      deplaceVersFaction: "République de Liberra",
      contenu:
        "Libris est bâtie sur un delta. La Lisdane s'y sépare en deux bras autour d'une longue péninsule — prolongement du continent, et non une île — et enserre trois îles. Tout ce qui compte dans la République tient sur ces quatre morceaux de terre et les cinq ponts qui les relient.\n\nLA PÉNINSULE\n\nLe Grand Marché (Quartier des Marchés)\nCœur commercial de la ville, occupant toute la péninsule. Entrepôts, comptoirs de change, guildes en tout genre — territoire naturel du Comptoir. C'est ici, dans l'arrière-boutique d'un entrepôt discret appartenant à un intermédiaire de confiance, que Maître Aurèle Ferrand traite ses affaires les moins avouables.\n\nLes Docks de Port-Libris (flanc est)\nPort principal, sur le bras vif. Les vaisseaux de commerce y sont tous passés quand le bras ouest s'est envasé, et les quais s'y sont agrandis d'autant. Lieu de passage pour toute intrigue impliquant contrebande, espionnage étranger, ou rumeurs fraîchement débarquées d'un autre continent.\n\nLa Rangée B (flanc ouest)\nLe Comptoir numérote ses parcelles d'entrepôt par rangées. Quand le bras ouest de la Lisdane s'est envasé, la Rangée B s'est retrouvée à cent pas d'un quai devenu inutilisable : le Comptoir a déclassé la rangée entière et revendu les parcelles à bas prix, plaques de laiton comprises. C'est aujourd'hui une rue de logements bon marché, d'ateliers et de tavernes installés dans des coques d'entrepôt trop grandes pour eux, face à des quais vides. Personne n'a rebaptisé les parcelles — on dit « je loge au B211 » comme ailleurs on dirait un nom de rue.\n\nLe B148\nAncien entrepôt à grain de la Rangée B, reconverti en taverne il y a douze ans par Bob, un porteur au dos cassé. La plaque de laiton du Comptoir est toujours boulonnée au-dessus de la porte, et le nom est resté : personne à Libris n'appelle l'endroit autrement. Salle unique trop haute sous plafond, charpente apparente, mezzanine louée à la nuit, étable aménagée dans une ancienne aire de chargement. Un incendie criminel jamais élucidé y a pris il y a deux ans, éteint de justesse par Henry, un gamin des rues que Bob a embauché le lendemain.\n\nLa Citadelle des Ponts\nCaserne principale de la Garde Citoyenne, sur l'éperon rocheux qui termine la péninsule. Les cinq ponts de Libris convergent à ses pieds : elle ne surveille pas la ville, elle tient ses articulations. Accès restreint, patrouilles visibles — un lieu qui rassure certains habitants et en inquiète d'autres.\n\nLES TROIS ÎLES\n\nLe Palais du Serment (île du sud-est)\nSiège de l'Assemblée et lieu de justice de la République, bâtiment le plus ancien de Libris — la charte fondatrice y est conservée sous verre, dans la salle où Vasnal préside encore les débats. Terrain neutre en théorie ; en pratique, les cinq blocs s'y affrontent à coups de procédure. L'île porte aussi un quartier ancien de toits de tuile qui dévale jusqu'à la rive. Ses quais n'accueillent que des barques personnelles, jamais du fret : on ne vient pas plaider à Libris avec une cargaison. Deux ponts la relient à la péninsule, l'un à la Citadelle, l'autre au Grand Marché.\n\nLe Quartier du Tissage (île du sud-ouest)\nTerritoire du Cercle des Peuples. Quartier mixte où se concentrent Aelindra, Mordanel exilés et autres non-humains de la ville : maisons rondes de bois tressé aux toits végétalisés, cours communes, ateliers, vergers, et un petit temple dédié à Aelindros en son centre. Un seul pont la relie à la Citadelle des Ponts — l'isolement politique du Cercle est aussi une donnée de terrain.\n\nLe Banc (île du nord-ouest)\nÎle basse au milieu des hauts-fonds, sur le bras mort. Le nom vient des bancs de sable ; personne à Libris ne fait semblant d'ignorer l'autre sens. On y trouve les marchands humains que l'envasement a déclassés en même temps que la Rangée B — des gens qui n'ont pas perdu leur métier mais leur quai, et qui regardent depuis vingt ans prospérer l'autre rive. Un seul pont, long, la relie au Grand Marché.\n\nLa Racine Dorée\nTaverne du Banc, repaire informel des Fils de Libris. Pas de siège officiel : leur pouvoir se construit ici, autour de bière bon marché et de discours populistes adressés à ceux que le Comptoir a ruinés.\n\nTERRAIN NEUTRE\n\nLa Table Commune\nAuberge-carrefour près des Docks, fréquentée par tout le monde — marchands, miliciens en repos, idéalistes désabusés, parfois même un Fils de Libris qui ne veut pas se faire remarquer. Le lieu neutre par excellence pour une rencontre discrète.",
    },
    {
      titre: "La Racine Dorée — un détail que personne n'a relevé",
      categorie: "Approfondissements par faction & peuple",
      deplaceVersFaction: "République de Liberra",
      mjSeulement: true,
      contenu:
        "L'établissement s'appelait La Racine Noire jusqu'à son rachat ; l'enseigne a été redorée à cette occasion, dans un mouvement dont l'insigne est un poing fermé sur une racine rouge sombre — et dont l'or est justement la couleur de l'Empire qu'il prétend combattre. Personne, chez les Fils de Libris, n'a relevé l'ironie.\n\nCf. la fracture des Fils de Libris (section Factions, République de Liberra) et le PNJ Ewen Castellan : sa troisième piste, la moins gardée, est cette même enseigne redorée peu après la période qu'il décrivait — une date que n'importe quel habitué du Banc peut situer au mois près. C'est la seule des trois voies vers le financement solvarien qui ne demande à personne de trahir sa hiérarchie. Secret de table : ne pas dévoiler tant que les PJ n'ont pas fait le lien eux-mêmes.",
    },
    {
    ...
    {
      titre: "La table — pain blanc, pain gris",
      categorie: "Vie quotidienne — la table",
      contenu:
        "Partout en terres humaines, la même opposition traverse les six nations, sans qu'aucune ne l'ait décidée.\n\n• Table haute : pain blanc (froment du Zénith), plats rôtis ou en sauce, service en plusieurs temps, sel en bloc posé sur la table.\n• Table basse : pain gris ou noir (orge, seigle), plat unique bouilli, tout dans le même pot, sel compté.\n\nCe n'est pas une question de goût mais de combustible et de main-d'œuvre. Un rôti demande un feu vif entretenu et quelqu'un devant ; une sauce demande du beurre, du temps et un cuisinier. Une bouillie demande une marmite et de l'oubli. C'est pour cette raison que la même fracture se retrouve d'un bout à l'autre du continent.\n\nCe qui distingue une nation d'une autre n'est donc pas la fracture elle-même, mais la façon dont elle module l'écart — et ce que cet écart dit du pays.",
    },
    {
      titre: "Gastronomie humaine — les six nations",
      categorie: "Vie quotidienne — la table",
      contenu:
        "Solvarn — l'écart maximal, par doctrine\n\nAucune cuisine intermédiaire : le Zénith mange pour être vu, la Marche mange pour tenir.\n• Zénith : Pain-soleil (froment et safran d'Autel, cuit au four de temple, marqué du soleil à huit branches) ; Sauce d'Or (bouillon de poule dorée, safran, miel, beurre monté) — on juge un cuisinier solvarien à sa nappe de sauce ; oie confite de la Sombre ; carpe de vivier au vin de Kor Valdan les jours de jeûne clérical, un jeûne qui coûte plus cher qu'un repas ordinaire ; un bol de poivre des quais laissé pur sur la table, qui n'est pas un plat mais un affichage de revenu.\n• Marche : la Grise (orge bouillie à l'eau, matin et soir) ; pain de Naros (seigle noir, tient trois semaines, ration réglementaire) ; soupe de fer (eau, chou fermenté, un morceau de lard si la semaine a été bonne) ; hareng de barrique dessalé. Un soldat solvarien sent le hareng, et il le sait.\n\nValdorne — la ligne passe par le gibier\n\nL'écart est juridique avant d'être économique : le gibier appartient au seigneur. Un paysan aisé ne mange jamais de cerf, un chevalier ruiné en mange encore.\n• Table de chevalerie : cerf-des-brumes rôti au cidre de Gardesèle, sanglier à la broche au retour de chasse.\n• Table paysanne : potée de navet-de-cendre et lard ; civet de lièvre, braconné, cuit porte fermée, les os brûlés après.\n• Le pommé (compote de pommes réduite des heures, sans sucre) est le seul plat que les deux tables partagent vraiment.\nConséquence de jeu : tout plat de gibier chez un roturier est un aveu.\n\nArveth — l'écart le plus faible\n\nPays mobilisé en permanence : la nourriture est une question logistique, pas une question de rang. Le noble arvethien mange exactement la même chose que ses hommes, simplement plus souvent et avec du sel en quantité. C'est un point d'orgueil national et une source de mépris réciproque avec Solvarn.\n• Galette de sarrasin, base commune du manoir à la cabane.\n• Le Chaudron de veille : bœuf des landes, orge, prunes de Boigris, cuit toute la nuit avant une bataille. On mange debout, dans le même chaudron, chacun sa louche.\n• Bœuf de l'année, fumé à l'herbe-à-fumée, pendu tout l'hiver — l'entamer trop tôt est un mauvais signe qu'on commente.\n• L'eau-de-vie de Boigris passe de main en main, même bouteille, du plus haut au plus bas gradé. Refuser est une insulte.\n\nMornac — la seule vraie gastronomie humaine\n\nMornac a eu ce que les autres n'ont pas eu : du temps et de la paix. Vallée fertile, fleuve navigable, pas de frontière chaude. C'est la seule nation humaine à avoir développé une cuisine pensée, avec un ordre de service, un vocabulaire propre et des cuisiniers qui portent un titre.\n• Jambon de Mornhaven, porc noir affiné trois ans, tranché à la main, servi seul.\n• Matelote de la Lisdane (anguille, vin noir, ail des levées) — le plat national, chaque famille prétend détenir la vraie version.\n• Quenelles de brochet : technique pure, impossible à réussir sans apprentissage, ce qui est exactement le propos.\n• Le Repas long : sept services, ordre fixe, du plus clair au plus sombre. Quitter la table avant le septième est une rupture diplomatique.\nUn cuisinier mornacien s'exporte : les grandes maisons de Solmaris en emploient, tout en les traitant de provinciaux jusqu'au moment où l'on goûte.\n\nServal — la cuisine à moitié naine\n\nServal vend le houblon, le bœuf de surface et la laine à Kaldrun, et lui achète le sel gemme et les techniques. C'est la seule cuisine humaine où l'on retrouve la cave, le fumoir et le bloc de sel sur la table.\n• Fromage de col, lait de mouton affiné en grotte murée — affinage emprunté aux nains, ce que Serval reconnaît volontiers et que Kaldrun rappelle constamment.\n• Agneau au fumoir, trois jours de fumée froide à l'herbe-à-fumée.\n• Bière de col, brassée à la manière naine : les nains la tiennent pour une contrefaçon et en achètent quand même.\n• Le bloc : sel gemme posé entier, on gratte soi-même. Seul endroit en terres humaines où le sel n'est pas servi mais partagé.\n\nLiberra — pas de cuisine, un carrefour\n\nIl n'existe pas de cuisine libérienne : il existe six cuisines dans la même rue, plus les épices d'import — et c'est l'argument identitaire de ses habitants. Deux exceptions nées vraiment là : l'anguille fumée du Grand Port (sur bois de pomme, mangée debout sur les quais) et le bouillon de silure, plat de banquet dont le seul fait de le servir affiche qu'on a financé l'expédition qui a pris la bête.",
    },
    {
      titre: "Gastronomie elfique — le végétal par défaut",
      categorie: "Vie quotidienne — la table",
      contenu:
        "On ne défriche pas, on prélève : pas d'élevage de masse, donc pas de viande quotidienne. La cuisine elfique ordinaire est entièrement végétale — farines d'arbre (gland, châtaigne), champignons, sèves, cueillette — avec le poisson comme intermédiaire toléré plus souvent.\n\nLa viande marque l'occasion, jamais le rang. Un prince aetharien et un cueilleur mangent tous deux du chevreuil le même jour de l'année, et aucun des deux les autres jours. C'est l'inverse exact du système humain, et les elfes en tirent une supériorité morale que les Solvariens trouvent insupportable.\n\nLe cerf blanc n'est jamais mangé.\n\nAetharion — le protocole\n• Sève claire bue pure en ouverture : la première coupe revient au plus âgé, en silence.\n• Truite de source crue, tranchée fine, sève ambrée et mousse-poivre.\n• Bouillon de trompette-de-sève, fond de toute la cuisine aetharienne.\n• Infusion de feuille d'argent en clôture : refuser la tasse est une insulte codifiée, avec une réponse codifiée.\n• Occasion : chevreuil aux baies, aux rites de nom. Deux ou trois fois dans une vie.\n\nAelindra — le plat partagé\n• Pain de gland doux, galette plate de tous les jours.\n• Fromage de feuille, serré dans des feuilles, mangé dans les trois jours — ne s'exporte pas, ce qui rend les marchands du Comptoir fous.\n• Le Plat : châtaigne, riz sauvage de Valmeryl, cresson, huile de noisette, un seul récipient où tout le monde mange. L'inverse exact du service aetharien, et c'est délibéré.\n• Occasion : sanglier gris boucané, aux fêtes d'automne.\n• Le miel de bosquet ne se cuisine pas : il se donne.\n\nMordanel — le nocturne\n• Soupe-lanterne : le bouillon garde la lueur du champignon une heure. On cuisine pour la lumière autant que pour manger.\n• Escargots de canopée grillés — les deux autres nations feignent d'être choquées et en redemandent en privé.\n• Vin de baie-de-crépuscule, très amer : ce que les autres prennent pour de la morbidité est de la précision.\n• Occasion : caille de brume, une semaine par an à la migration. Toute la communauté cuisine en même temps — c'est un événement, pas un repas.",
    },
    {
      titre: "Gastronomie naine — la levure et le sel",
      categorie: "Vie quotidienne — la table",
      contenu:
        "Pas de soleil, donc pas de variété : toute la sophistication naine s'est reportée sur la fermentation (chaque clan sa souche, jamais interrompue) et sur le sel gemme.\n\nLa ligne de fracture n'est pas riche/pauvre mais politique. À Kaldrun, manger de la surface est une réussite ; dans les Failles, manger ce que la Faille donne est une preuve de sang. La même assiette n'a pas le même sens à deux jours de marche.\n\nKaldrun — l'Ordre\n• Bouillie de cave-mousse au fromage bleu de chèvre de galerie : le quotidien, que personne ne prétend aimer.\n• Champignon-barrique tranché épais et saisi sur plaque, en substitut de viande.\n• Pain de souche : le pain porte le goût de la famille. Servi aux funérailles, il dit qui enterre qui.\n• Bière de clan, brassée au houblon acheté à Serval — seul point de dépendance à la surface, et sujet de politique intérieure permanent.\n• Prestige : bœuf de surface descendu vivant par les rampes, et sombre-truffe râpée sur la bouillie ordinaire. La richesse naine se démontre par ce qu'on importe, pas par ce qu'on cuisine.\n\nKhazrak Dûm — les Failles\n• Chapeau-rouge bouilli trois fois : le manger est une déclaration d'appartenance évolutionniste. Mal bouilli, il rend malade, et c'est admis.\n• Ver-de-roche juvénile grillé au poivre-de-cendre, délicatesse renégate et répulsif absolu pour l'Ordre.\n• Graisse de ver : cuisine et lampes, le même bidon sert aux deux — ce qui scandalise les visiteurs de Kaldrun.\n• Rat des veines, grillé sans façon. Viande ordinaire ici ; à Kaldrun, l'accusation d'en manger est une insulte grave. La même bête, deux mondes.\n\nLes trois rites de table\n• Champignon-écho avant les serments de clan et les chants de forge : il grave la voix une heure. On ne jure pas d'une voix ordinaire.\n• Poisson-cloche aux mariages, servi encore chantant. Certains clans trouvent ça barbare et le disent, ce qui est en soi une prise de position.\n• Pain de souche aux funérailles. Un clan qui a perdu sa levure ne peut plus enterrer correctement les siens : voler une souche est un casus belli, pas un vol.",
    },
    {
      titre: "Notes MJ — décrire une auberge",
      categorie: "Vie quotidienne — la table",
      mjSeulement: true,
      contenu:
        "Outil de description immédiate, utilisable dans n'importe quelle ville humaine.\n\n• Quelques pièces de cuivre : bouillie ou soupe du pot, pain gris, bière coupée d'eau. Le pot est sur le feu depuis ce matin et il y restera demain.\n• Argent : un plat cuisiné de la région (matelote, potée, agneau fumé), pain gris frais, vin ou bière du pays. C'est là que les personnages mangent normalement.\n• Or : rôti ou plat en sauce, pain blanc, vin nommé, poivre sur table. L'auberge s'est procuré du froment et un cuisinier — dans une petite ville, elle l'a fait pour quelqu'un en particulier, et les joueurs peuvent demander pour qui.\n\nTrois détails suffisent à situer une table sans rien expliquer : la couleur du pain, le fait que le sel soit servi ou compté, et le fait que le plat arrive dans un pot ou dans une assiette.\n\nFils narratifs disponibles :\n• Le gibier chez un roturier valdornien est un délit visible dans l'assiette.\n• Les cuisiniers mornaciens employés dans les grandes maisons du Zénith forment un réseau d'oreilles que personne n'a jamais pensé à surveiller.\n• Une souche de levure naine volée est un motif de guerre entre clans, et un objet de quête qui tient dans une poche.",
    },
    /* Refonte magie & Remous — décisions délibérées, ne pas « corriger » sans
       relire prompt_systeme_magie_remous.md :
       - « Source unique, trois prismes » est supprimé : les Humains n'ont
         jamais eu d'accès natif à l'Arbre, ils puisent dans la Mer des
         âmes. Le prisme humain est renommé « Sang divin → magie de la Mer »
         (le nom Sang divin reste employé par l'Église, à dessein trompeur).
       - Compensation des casters de source Mer VOLONTAIREMENT différée
         jusqu'aux paliers de Déchirure ci-dessous. Ne pas ajouter de bonus
         compensatoire ailleurs (jets, dégâts, PP) pour "équilibrer" la
         magie humaine face à la Sève/Pierre/Runes de sève : la contrepartie
         est ici, dans la jauge de Remous, et elle est volontairement
         empoisonnée (plus la magie est facile, plus elle attire un démon).
       - Repère de calibrage (calculerPPMax() = 4 + niveau×2 + mod×2, ~16 PP
         à niveau 3) : le groupe actuel (un magicien, deux enchanteurs)
         dispose d'environ 48 PP de source Mer cumulés. En membrane épaisse,
         atteindre le palier 1 exige de vider toute la réserve collective
         sans bouger ni se reposer — les paliers 2 à 4 sont hors de portée
         avant un bon niveau. En membrane déchirée (l'archipel Sylvareth),
         les quatre paliers tiennent dans cette même réserve : un groupe de
         niveau 3 pourrait y créer une Plaie dès sa première visite, et
         2d6/3d6 dégâts par tour tuent un magicien d4 en deux ou trois tours.
         C'est un signal de contenu haut niveau, pas un accident
         d'équilibrage — ne pas revoir les seuils à la hausse pour "protéger"
         un groupe de bas niveau qui s'y rendrait trop tôt.
       - Les démons sont immunisés aux dégâts de Déchirure (cf.
         data/bestiaire.js, capacitesSpeciales) : ils sont faits de cette
         eau, la leur retirer les tuerait de leurs propres manifestations.
       - Dette de conception assumée : trois des sept personnages du groupe
         sont des casters humains ; le barde nain et les combattants ne
         génèrent aucun Remous. Le canon affirme que la magie humaine est
         "plus puissante que la Sève" sans que cet avantage soit chiffré
         nulle part — différé volontaire, décision explicite de Thomas.
    */
    {
      titre: "La membrane et les Remous",
      categorie: "Notes MJ",
      mjSeulement: true,
      contenu:
        "Principe\n\nChaque sort de source Mer agite la Mer des âmes. Le Remous est une accumulation, pas un jet de dé : quand il atteint le Seuil de Déchirure, la paroi cède et un démon apparaît. Aucun hasard — le système est délibérément prévisible, pour qu'une table qui suit son compteur puisse décider de s'arrêter ou de continuer en connaissance de cause.\n\nLa jauge de Remous\n\n• La jauge appartient au lieu, pas au lanceur. Tous les casters de source Mer présents alimentent la même eau. Une jauge par lieu, additive.\n• Magie filtrée par un dieu correctement ancré : contribution comptée à moitié, arrondi à l'inférieur.\n• Magie de source Arbre (Sève, Pierre, Runes de sève) : ne contribue pas.\n• Vidange : repos long, ou une heure de marche effective. L'eau se calme quand on s'en va — le Remous n'est pas un compteur de carrière, c'est l'agitation d'un endroit précis.\n• À la déchirure : un démon D1 se manifeste (famille au choix du MJ, cf. « Les familles démoniaques ») et la jauge continue de monter — les seuils sont cumulés, elle ne se vide que par repos long ou une heure de marche effective.\n\nLes quatre paliers de Déchirure\n\nLa Déchirure n'est pas qu'une sanction : c'est une vanne. Plus on approche de la Mer, moins la magie coûte et plus elle frappe — et c'est précisément pour ça qu'aucun mage humain sensé ne s'arrête au bon moment. Le système tente, il ne punit pas.\n\n• Palier 1 — Déchirure (48 PP en membrane épaisse) : coût des sorts -1 PP, +2 dégâts. Coût : 1 démon D1.\n• Palier 2 — Ouverture (96 PP) : coût -2, +4 dégâts, PP max +4. Coût : 1d6 dégâts par tour à tous, et le démon monte d'un palier.\n• Palier 3 — Marée (144 PP) : coût -3, +6 dégâts, PP max +8. Coût : 2d6 dégâts par tour à tous, le démon monte d'un palier, et un second démon apparaît.\n• Palier 4 — La Plaie (192 PP) : coût -4, +8 dégâts, PP max +12. Coût : 3d6 dégâts par tour à tous, le démon atteint son plafond, et le lieu reste déchiré.\n\nPlancher de coût : 1 PP. La réduction ne rend jamais un sort gratuit.\n\nLes dégâts par tour frappent tout ce qui se trouve dans le lieu — alliés, non-casters, ennemis compris. Conséquence assumée : un groupe acculé peut délibérément agiter l'eau pour saigner toute une salle. Coûteux, suicidaire à long terme, rationnel une fois.\n\nLes démons sont immunisés aux dégâts de Déchirure — ils sont faits de cette eau. Sans quoi la déchirure tuerait ses propres manifestations.\n\nLa Déchirure est une porte ouverte, pas un événement fini. Tant que le total dépasse le seuil courant, l'ouverture entre la Mer des âmes et le monde matériel tient : le démon qui en est sorti n'est pas le dernier qui puisse en sortir. Elle se referme quand le palier retombe à zéro — ce qui s'obtient en cours de scène en épaississant la membrane, jamais en vidant la jauge. Un Voile de sève planté sur place fait passer le lieu au cran au-dessus : le total ne bouge pas, le seuil monte, le palier retombe, et la porte se ferme. Ne pas annoncer qu'elle est ouverte — décrire l'air trop mince à un endroit de la pièce et laisser les lanceurs le sentir.\n\nPersistance : les paliers sont acquis pour la scène et retombent avec la jauge (repos long ou une heure de marche effective). Exception : la Plaie. Le palier 4 laisse le lieu à membrane déchirée pour longtemps — un nouveau Valmoire créé par les joueurs eux-mêmes. Réparable — cf. « Sceller n'est pas refermer » ci-dessous pour les deux voies, très différentes, d'y remédier.\n\nLe rituel Sylvareth contre Ysmaal n'était au fond rien d'autre qu'une Déchirure poussée jusqu'au bout. L'archipel est le cratère d'une Plaie.\n\nSeuils par membrane\n\n• Épaisse (×1 — paliers 48 / 96 / 144 / 192) : le monde ordinaire, Libris, la route.\n• Amincie (÷2 — paliers 24 / 48 / 72 / 96) : champs de bataille, charniers, Académies de Liberra, Solmaris.\n• Fine (÷3 — paliers 16 / 32 / 48 / 64) : Valmoire, ruines de la Fracture, sites de massacre ancien.\n• Déchirée (÷4 — paliers 12 / 24 / 36 / 48) : l'archipel Sylvareth.\n\nConséquence de monde — l'archipel\n\nLa magie y est presque gratuite et les démons y pullulent. Un mage humain qui y débarque se découvre bien plus puissant qu'ailleurs et comprend trop tard pourquoi. Ça donne aux Sylvareth une raison de plus d'y être coincés, et ça explique qu'aucun comptoir ne s'y installe malgré la position stratégique sur les routes maritimes.\n\nLe cas de la prêtresse naine\n\nLa magie divine est de la Mer filtrée par un dieu, quelle que soit la race du prêtre. La prêtresse naine du groupe contribue donc au Remous à moitié, et non zéro — c'est le même fait qui lui donne le canal permettant de détruire définitivement un démon.\n\nConséquences de monde\n\n• Une ville de mages est une plaie. Tout lieu de pratique dense a une membrane amincie en permanence — seconde raison, pragmatique celle-là, du verrouillage solvarien de la pratique.\n• Valmoire s'explique sans réécriture. L'incarnation de la Traque reste canonique ; la membrane fine explique pourquoi là et pas ailleurs. L'Empire a donc bâti son accusation contre les Mordanel sur un fait géologique. L'ambiguïté canonique — mensonge ou conviction sincère — est préservée et enrichie.\n• La méfiance des laïcs de Liberra envers les mages est rationnelle, pas superstitieuse.\n\nSceller n'est pas refermer\n\nSceller — les pylônes\n\nUne faille peut être scellée, jamais refermée, au moyen de pylônes combinant sève et Grisfer. Le scellement contient, il ne guérit pas : la membrane reste déchirée sous le sceau.\n\nAucune puissance ne peut sceller quoi que ce soit seule. La sève est produite par les Aelindra (Cercle des Peuples) ; le Grisfer est un monopole de Kaldrun (alliée de l'Empire). Chaque Plaie ouverte devient donc mécaniquement une table de négociation entre des gens qui ne se parlent pas — et Liberra, qui creuse aux Brisants, sera la première à devoir demander aux deux à la fois.\n\nRefermer — le rituel\n\nUne faille ne peut être réellement refermée que par le sacrifice d'une Grande Dryade. Les Grandes Dryades sont rarissimes, quasi-divines, gardiennes chacune d'un site sacré (cf. section « Les Dryades — gardiennes de l'Arbre »).\n\nAetharion les protège étroitement — et son isolationnisme, déjà connu de tous pour d'autres raisons, en porte désormais une de plus, jamais dite : les Hauts Elfes ne gardent pas une relique, ils gardent un remède qu'on voudra leur arracher. Une nation acculée finira par y penser, et Aetharion le sait déjà.\n\nLe remède existe, il est connu, et c'est un meurtre. Personne ne l'adoucit en le nommant autrement.\n\nUn désaveu qui n'était pas symbolique\n\nLe canon indique que les dryades proches des terres Mordanel se sont en partie retirées après le Silence de Valmoire (cf. section « Le Chaos — hiérarchie et manifestations »). Avec cette règle, ce retrait cesse d'être un simple désaveu symbolique : une région qu'une Grande Dryade a quittée a perdu, du même coup, sa seule possibilité de guérison.",
    },
    {
      titre: "Les familles démoniaques",
      categorie: "Notes MJ",
      mjSeulement: true,
      contenu:
        "Qui digère les âmes\n\nLes trois Enfants de la Sève bannis dans la Mer sont Aetharyn (Lumière & Temps), Sylvath (Sève & Croissance) et Khoreth (l'Avidité). Nyaleth (Maladie & Putréfaction), le seul dieu tombé encore libre, n'est pas enfermé dans la Mer mais y puise quand même : il dispute les âmes aux trois bannis. Ses restes de repas en ressortent comme les leurs — ce sont des démons, pas des morts-vivants.\n\nChaque démon garde la marque de celui qui l'a digéré :\n• Sylvath → Guerre : une chair qui repousse et s'arme, des griffes, la survie. Craint le fer, se rit de la magie.\n• Aetharyn → Sorts : la lumière qui brûle, l'instant dérobé. Fragile, frappe fort à distance, se nourrit des PP.\n• Khoreth → Tentation : la séduction, le désir irradié, les liens rompus.\n• Nyaleth → Endurance : la putréfaction, la lenteur. Très résistant, tue sur la durée, jamais d'un coup.\n\nAucun démon ne parle, quelle que soit sa famille ou son palier : il irradie, il ne discourt pas.\n\nCe que les joueurs ne doivent jamais recevoir\n\nAucun PNJ ne relie un démon à un dieu. La correspondance est découvrable (érudit, prêtre, observation répétée), jamais donnée. Un démon de Tentation ne porte jamais la signature de Khoreth — ni dette, ni compte, ni objet convoité, ni le manque dont elle se nourrit — sans quoi un joueur attentif remonte jusqu'au Roi des Mendiants.\n\nPaliers\n\n• D1 à D3 : formes génériques de chaque famille.\n• D4 : formes personnalisées, au moins deux par famille. Celle que prend un démon qui monte est au choix du MJ.\n• D5 : grands démons à part entière, au moins deux par famille. Jamais atteints par escalade — le MJ les place.\n\nChaque famille monte par sa propre faim (PV infligés, PP drainés, états de contrôle réussis, rounds survécus), environ trois repas réussis par palier. Le Remous garde son rôle : ses paliers font aussi monter le démon, jusqu'à D4 au plus.\n\nÀ 0 PV, le démon retourne à la Mer et s'y reforme : rien ne le suit. Un démon qui ressort plus tard est une nouvelle manifestation. Seule la Destruction définitive l'efface pour de bon.",
    },
    {
      titre: "La Lisdane — ce qu'on tire du fleuve",
      categorie: "Vie quotidienne — la table",
      contenu:
        "Libris mange le fleuve. Ce qui arrive sur les étals du Grand Marché vient d'abord de la Lisdane, et un marché de Libris se lit comme une carte : chaque chose y dit d'où elle a été descendue.\n\nEN DESCENDANT, EN REMONTANT\n\nL'anguille et le silure tiennent le bas du fleuve, autour de la ville et jusqu'à l'embouchure. Le brochet remonte beaucoup plus haut dans les terres, mais il s'arrête net là où le fleuve se casse en coteaux — les pêcheurs disent qu'on ne prend pas de brochet au-dessus des vignes. Au-dessus, précisément, commence la vigne noire, qui donne le vin de Mornac : en remontant, la première vigne noire dit qu'on a quitté Liberra.\n\nLES LEVÉES\n\nEntre les deux, des dizaines de lieues de levées — des digues de terre plantées de génération en génération, où poussent la poirée en rangs et l'ail des levées. Les crues en ouvrent une tous les dix ou vingt ans. Quand une levée cède, on ne la replante pas : la terre derrière ne vaut plus rien, les gens s'en vont, et le fleuve garde le morceau.\n\nMais la poirée, elle, continue. Elle repousse seule pendant des années après qu'on a cessé de la tailler, et elle garde ses rangs. Des rangs droits au milieu d'une friche, sur une levée abandonnée, veulent dire qu'il y avait un jardin là et qu'il n'y a plus personne pour s'en occuper. Les gens du fleuve les lisent sans y penser ; les gens de la ville passent devant sans les voir.\n\nCE QUI POUSSE OÙ ÇA NE DEVRAIT PAS\n\nDeux plantes du fleuve sont des anomalies, et les cuisiniers de Libris les connaissent bien.\n\nLa racine-de-lune ne se laisse déterrer que de nuit et noircit au soleil en une heure. Elle coupe la faim un jour plein, ce qui en fait la ration des contrebandiers et des déserteurs — et son défaut est exactement ce qui l'empêche depuis toujours d'être une culture. Plus d'un jardinier s'y est usé.\n\nLa barbade pâle, ensuite, qu'on achète en infusion aux comptoirs nains. Ce n'est pas une variété : c'est de la barbade ordinaire poussée sans soleil, sous lampes runiques, et elle en sort blanche. Elle ne peut donc exister que sous terre, dans les Failles ou les galeries de Kaldrun. De la barbade blanche poussée à l'air libre n'aurait aucun sens — à moins qu'il y ait une lumière quelque part sous les pieds.",
    },
    {
      ...
      mjSeulement: true,
      contenu:
        "Avant l'envasement de la Lisdane, le Comptoir stockait son grain sous terre : des celliers voûtés courant sous toute la Rangée B et jusque sous les entrepôts du Grand Marché — c'est la même péninsule et la même pierre —, frais et secs, reliés entre eux pour qu'on puisse déplacer les sacs sans remonter. Au déclassement de la rangée, les accès ont été murés parcelle par parcelle, aux frais des nouveaux propriétaires. Le réseau, lui, est intact — et il n'a jamais été vraiment vide.\n\nC'est là que siège le Roi des Mendiants, grand hôte de la Cupidité, dans les celliers d'un quartier construit pour garder ce qu'on a peur de manquer. Khoreth n'aime que l'objet possédé, jamais le geste qui le produit : le grand hôte loge sous la richesse, jamais dedans — il couve ce qu'il ne produit pas.\n\nSon économie : il ne touche jamais l'or. Il prend des promesses, des services, des noms. Un porteur qui a reçu un repas un mauvais hiver ne rend jamais ce repas : il rend, une fois, un renseignement, et il ne se considère toujours pas quitte. Une dette qu'on ne solde pas vaut mieux qu'une dette qu'on rembourse. Conséquence de table : le levier qui fonctionne sur un cupide ordinaire — lui jeter de l'or pour le détourner — le nourrit.\n\nSon réseau, ce sont des enfants. Les bandes d'orphelins de Libris, les Doigts, collectent ce qu'elles entendent et sont payées en repas, en couvertures, en petites faveurs, jamais en pièces. Un enfant est invisible dans un marché : il traîne, il écoute, personne ne baisse la voix devant lui. Les Doigts ignorent pour qui ils travaillent — ils connaissent un intermédiaire, jamais deux.\n\nSes rapports avec les cinq blocs. Comptoir : il connaît les vrais livres mieux que Selyne Orwick, parce que les porteurs lui doivent quelque chose. Garde Citoyenne : elle utilise les positions de mendiants comme informateurs, et le Lieutenant Osgoode n'a jamais demandé qui les attribue. Fils de Libris : il possède les pauvres dont Dessalles prétend parler, Dessalles l'ignore, aucun contact, rivalité purement structurelle. Serment de Libris : sans intérêt pour lui, il ne veut pas le pouvoir, il veut le manque. Cercle des Peuples : son seul angle mort — le Tissage s'occupe de ses propres pauvres, personne là-bas ne lui doit rien.\n\nCe dernier point compte : le Quartier du Tissage est la seule communauté de Libris qui n'a pas besoin de lui, et une émeute qui la briserait lui rendrait service. Il n'organise pas le pogrom. Il ne fera simplement rien pour l'empêcher, alors qu'il est de tous les habitants de cette ville celui qui saurait le plus tôt qu'il se prépare. Sa participation est un silence — et c'est exactement ce qu'on pourra lui reprocher plus tard.\n\nRègle de mise en scène : il n'apparaît jamais avant le dernier acte. On ne le voit pas, on voit ses positions. Un mendiant toujours au même coin. Un gamin qui sait trop. Une porte murée. Un porteur qui refuse un pourboire.\n\nPOURQUOI MAINTENANT, APRÈS TRENTE ANS\n\nLes créances ne sont pas récentes : la moisson date de l'Envasement. En déclassant la Rangée B, le Comptoir a produit en une décennie plusieurs centaines de gens qui avaient besoin de quelque chose tout de suite, et quelqu'un est passé le leur donner. Trente ans de dettes dormantes, jamais réclamées — une dette qu'on ne solde pas vaut mieux qu'une dette qu'on rembourse.\n\nElles sont appelées cette année parce que la guerre a rendu disponible une pénurie d'une ampleur qu'aucune ville n'avait offerte jusque-là. Khoreth ne se nourrit pas de l'or : elle se nourrit du manque et de l'avidité qu'il déclenche. Une ville qui thésaurise produit plus de son concept en un mois qu'une ville riche en dix ans. Le Roi des Mendiants ne réclame donc pas ses dettes pour s'enrichir : il les dépense en main-d'œuvre pour fabriquer la disette qui alimente son incarnation (cf. « Notes MJ — éléments en construction »).\n\nCorollaire à tenir en toutes circonstances : il ne veut pas que Libris meure de faim. Il veut qu'elle croie qu'elle va mourir de faim, et le plus longtemps possible. Toute action des personnages qui rassure réellement la ville lui coûte plus cher qu'un coup porté à ses hommes — et c'est le seul levier contre lui qui fonctionne sans qu'on ait besoin de savoir qu'il existe. Cela vaut tant que la Fièvre n'a pas atteint son dernier palier : la disette est une rente aussi longtemps qu'elle dure, et une rente ne se dépense qu'une fois. Ce n'est qu'à l'émeute qu'il cesse de la faire durer — et ce jour-là, ce qu'il en tire n'a plus rien à voir avec ce qu'il en tirait la veille.\n\nLa galerie basse. Le colosse charognard n'a pas été détruit : il tient toujours la galerie que la marée noie deux fois par jour. La communauté des Fondations le sait et le contourne. C'est une horloge qui n'avance pas tant que personne ne la remonte.\n\nCE QUE PRODUIT UN SIGNALEMENT À LA GARDE\n\nRien de ce qu'imagine celui qui signale. La Garde Citoyenne n'a pas six hommes à envoyer sous terre et le sait ; le Lieutenant Osgoode transmettra, et la transmission ira où vont toutes celles de ce genre — vers le propriétaire. Les celliers appartiennent légalement au Comptoir. Un rapport sur du grain détourné dans une cave du Comptoir cesse d'être une affaire de police pour devenir une affaire de propriété, et ce sont Selyne Orwick puis Draeven qui apprennent l'existence des Fondations.\n\nLe Roi des Mendiants n'a rien à faire contre ça. La consigne a déjà fonctionné une fois : quand quelqu'un redescendra, les salles seront vides, le grain aura glissé d'une rangée, et les seules personnes trouvées en bas seront celles qui ne marchent pas assez vite. Il ne perd pas un réseau dans cette affaire : il perd un entrepôt et gagne cent personnes qui viennent d'apprendre que la République descend les chercher.\n\nNe pas résoudre ça en une scène. Le délai est le sujet — il doit s'écouler, entre le signalement et le premier effet visible, assez de temps pour que les personnages croient que rien n'arrivera.",
    },
    {
      titre: "Notes MJ — le B0, le marché sous le Grand Marché",
      categorie: "Notes MJ",
      mjSeulement: true,
      contenu:
        "Le marché noir de Libris se tient dans les celliers voûtés du Comptoir, sous les entrepôts du Grand Marché. On y entre par une trappe de chargement dont la parcelle change toutes les quelques semaines. La salle est fraîche et sèche, éclairée à la lampe basse ; on y vend ce qui n'est sur aucun registre — de la Pleine quand il y en avait, des lampes naines dépareillées, des licences périmées, des curiosités descendues des Docks.\n\nLE PASSAGE\n\nLe Comptoir numérote ses parcelles d'entrepôt par rangées, et numérote à partir de un. Ceux qui savent appellent l'endroit le B0 : la parcelle zéro, celle qui n'est sur aucun registre. Le nom est la première moitié du passage, la seconde est une épreuve de tempérament.\n\n— Je cherche le B0.\n— Y a pas de B0.\n— Je sais.\n\nN'importe qui peut répéter « B0 » s'il l'a entendu dans une taverne, et le portier le renvoie d'un mensonge banal. Celui qui insiste, argumente, montre une preuve ou nomme un vendeur se trahit : il croit qu'il négocie. Le bon visiteur acquiesce. Aucun jet — un joueur inattentif peut échouer avec la bonne information en main, et il pourra revenir le lendemain. Forcer une trappe ne donne qu'un cellier vide : le marché se déplace, le mot de passe est ce qui reste stable.\n\nLE TENANCIER\n\nUn homme d'une soixantaine d'années tient la salle depuis assez longtemps pour que personne ne se rappelle avant lui. Il ne vend rien lui-même : il attribue les places, règle les différends, décide qui entre. Il vit là, dans une pièce attenante — un lit, une table, des vêtements sombres et propres pendus au mur en plusieurs tailles. Poli, lent, sans menace, désagréable à personne.\n\nC'est le Roi des Mendiants. Le grand hôte de la Cupidité loge sous la richesse et jamais dedans : il couve ce qu'il ne produit pas. Le marché n'est pas sa couverture, c'est sa maison, et les personnages vont y faire leurs courses.\n\nIl ne touche jamais l'or — c'est le cœur de toute scène qui se joue ici. Il vend ce qu'on lui demande, puis propose un autre arrangement, moins cher, plus simple, sans échéance : « vous me revaudrez ça ». Accepter, c'est entrer dans le registre de l'entrepôt n° 7 par la grande porte, et personne à table ne verra que c'est ce qui vient de se passer. Un groupe qui surpaie pour se débarrasser de lui obtient tout, immédiatement et de bonne grâce : c'est la pire chose qui puisse lui arriver et ça ressemble à une victoire.\n\nC'est lui qui prête le premier habit sombre. Le porteur le prête à son tour, et chacun croit rendre service à celui qui le lui a remis. Les vêtements au mur n'ont aucune signification pour qui n'a pas encore croisé d'habit sombre : ne jamais les souligner ; s'ils reviennent plus tard, ils auront toujours été là.\n\nTROIS INTERDITS\n\nNe jamais le nommer ni le désigner. Il ne dit pas qui il est, ne fait aucune allusion, n'a aucune prestance. Il n'y a pas de révélation dans cette salle : il y a un tenancier, et c'est tout ce que la table doit pouvoir en dire en sortant.\n\nNe jamais récompenser la filature. Suivre un habit sombre mène à un débiteur, qui mène à un autre, qui ne se souvient plus. Explorer les galeries donne un mur, puis un autre : le réseau est muré parcelle par parcelle depuis trente ans.\n\nNe jamais confirmer la bonne hypothèse. Si la table finit par se dire que le marché est la maison de quelqu'un, la laisser courir sans un mot. Une hypothèse de joueurs reste une hypothèse ; une confirmation du MJ devient une entrée de bestiaire. Si elle lui trouve un sobriquet, l'adopter immédiatement.\n\nCE QUE LA SALLE RELIE\n\nC'est ici qu'a abouti le coffret de plomb à cire noire, et c'est ici que trois étudiants de l'Atelier des Marges l'ont acheté. Le tenancier ne tient pas de livre ; les vendeurs de la salle, eux, se souviennent de qui l'a apporté. C'est aussi ici que se revendent les petits objets volés en ville, et ici qu'on se plaint à voix haute que la Pleine ne descend plus depuis des semaines.",
    },
    {
      titre: "Notes MJ — les hommes en habits sombres",
      categorie: "Notes MJ",
      mjSeulement: true,
      contenu:
        "Ce n'est pas un homme. C'est un habit.\n\nLa description revient à l'identique chez des témoins qui ne se connaissent pas : environ six pieds, vêtements sombres et propres, cheveux sombres, ni bijou ni arme visible. Il passe, il demande, il s'en va. Il ne reste jamais dormir.\n\nLa constance ne vient pas d'un homme, elle vient du vêtement. Ce sont des débiteurs ordinaires — un porteur, un batelier, un ancien commis — à qui l'on prête l'habit le temps d'une visite, avec ce qu'il faut dire. Ils ne se croisent jamais. Aucun n'a vu le Roi des Mendiants : chacun croit rendre un service à l'homme qui lui a fait porter l'habit, et ignore que celui-là le portait aussi.\n\nConséquence : l'habit est le seul témoin fiable, et il ne mène nulle part. Un portrait-robot sera parfaitement exact et parfaitement inutile. Qui suit cette piste suit un manteau.\n\nLE PIÈGE DE TABLE\n\nTrois témoignages concordants produisent inévitablement la conclusion « c'est le même homme » — et cette conclusion devient fausse au moment précis où elle devient satisfaisante. Ne pas la contredire. La laisser courir, et laisser un fait la démentir plus tard : deux habits sombres vus le même soir dans deux quartiers, et une seule ville pour les contenir.\n\nNe jamais donner de sobriquet à ce personnage. Si la table en invente un, l'adopter immédiatement : un nom trouvé par les joueurs reste une hypothèse de joueurs, un nom donné par le MJ devient une entrée de bestiaire.\n\nLES TROIS DEMANDES\n\nÉventrer les convois sans les piller. Déplacer des sacs d'un entrepôt vers un autre, légalement. Répéter au bon comptoir ce qui fera parler la ville. Trois débiteurs différents, qui n'ont aucune conscience d'exécuter la même opération : fabriquer le manque, puis en récolter la colère par les Fils de Libris. Aucun d'eux ne trouve sa tâche extraordinaire, et c'est ce qui rend la chaîne indétectable — on n'interroge pas un homme qui a déplacé des sacs.\n\nLa moisson de l'Envasement n'est pas la seule. Bob a muré sa cave il y a douze ans, pas trente : la pratique est continue, l'Envasement en fut seulement la meilleure année. Un PNJ n'a donc pas besoin d'avoir été ruiné en même temps que les autres pour devoir quelque chose — la question à se poser pour n'importe quel habitant de la Rangée B est : de quoi avait-il besoin, son pire hiver ?\n\nLES NOMS RECONNUS\n\nLe registre porte une centaine de bénéficiaires, et les personnages en ont reconnu plusieurs : le fournisseur de Bob, des familles de la Rangée B qu'ils croisent depuis des mois. Ces noms sont des voisins, pas des complices — aucun d'eux ne sait qu'il figure sur une liste, aucun n'a rendu de service, aucun n'a rencontré d'homme en habits sombres. Recevoir n'est pas encore devoir : la dette se contracte le jour où l'on accepte, et se révèle des années plus tard.\n\nConsigne : ne jamais transformer un nom reconnu en informateur. La tentation sera forte d'en faire un fil à tirer ; le sujet de cet arc est justement qu'il n'y en a pas. Chaque nom confirme la méthode et n'ajoute rien à l'enquête, et c'est ce qui doit finir par inquiéter la table.",
    },
    {
      titre: "Le commerce de Libris — toile de sève et limon",
      categorie: "Approfondissements par faction & peuple",
      contenu:
        "Libris ne produit presque rien. Elle transporte, elle convertit, et elle facture la différence — et l'essentiel de sa fortune tient à une seule marchandise qu'elle ne fabrique pas.\n\nLA TOILE DE SÈVE\n\nLes Aelindra cultivent des plantes à fibre nourries de sève et en tissent une étoffe. Le Quartier du Tissage ne porte pas son nom par hasard. Appliquée sur la peau, la toile absorbe une part de ce que le Chaos dépose : pansements qui empêchent une blessure corrompue de tourner, doublures portées sous l'armure, bandes que les chirurgiens réclament à n'importe quel prix. Ce n'est pas un luxe, c'est un consommable — on l'use, on la rachète.\n\nElle n'efface rien de définitif. Ce qui s'est inscrit dans une âme y reste. Elle achète du sursis, et le sursis se vend cher.\n\nLA MEMBRANE, SANS QUE PERSONNE NE LE SACHE\n\nCe que la toile fait à l'échelle d'un pansement, elle le fait aussi à l'échelle d'un lieu : tendue plutôt que portée, elle stabilise la membrane entre Eldoria et la Mer des âmes, et peut la remonter d'un cran (déchirée → fine → amincie → épaisse — cf. Notes MJ, « La membrane et les Remous »). C'est la seule raison pour laquelle les villes humaines ne sont pas submergées de démons : Solvarn achète, sans le savoir, son assurance anti-démons à des elfes que sa propre doctrine qualifie de « démons elfiques ».\n\nLes Aelindra tiennent seuls la production. Le Cercle des Peuples dispose donc, à l'Assemblée de Libris, d'un levier que personne — pas même le Comptoir — n'a encore songé à chiffrer.\n\nLa sève vient de l'Arbre, et l'Arbre dépérit. La seule protection réelle du monde contre ce qui remonte de la Mer est donc une ressource en voie d'épuisement lente — sans échéance connue, mais pas infinie.\n\nPOURQUOI LIBRIS EST RICHE\n\nL'Empire de Solvarn est une théocratie de la pureté qui purge le Chaos par le feu. Ses soldats en meurent, ses inquisiteurs en ont besoin, et son clergé ne peut absolument pas être vu achetant quoi que ce soit à un elfe. Alors il achète à Libris. La toile part du Tissage, traverse la péninsule, et ressort des Docks de Port-Libris sous une marque liberrane — « toile de Libris » sur le connaissement. Personne n'a menti : elle est bien passée par Libris.\n\nLes inquisiteurs de Solvarn portent de la doublure elfique sous le tabard, et le Comptoir encaisse la différence entre le prix d'achat et le prix du mensonge. « Le fleuve ne choisit pas ses passagers » n'est pas une philosophie : c'est ce que le Comptoir vend.\n\nLa même étoffe sert aux deux camps. Ceux qui subissent le Chaos en portent pour tenir ; ceux qui l'emploient en portent pour s'y enfoncer un cran plus loin. Le Comptoir vend aux uns comme aux autres et ne pose de question à personne.\n\nCE QUE LIBRIS VEND EN RETOUR\n\nLes cultures de sève épuisent le sol, et les Aelindra ne peuvent pas fabriquer de la terre. Le delta de la Lisdane, lui, en produit par barges : le Comptoir drague le bras mort et vend le limon aux Aelindra, qui paient cher ce qui régénère une terre morte. S'y ajoute, en volume, le métal nain en transit — les Aelindra ne minent pas, et Khazrak Dûm comme les Nains de l'Ordre commercent déjà à Libris.\n\nLe limon est la signature, le métal est le tonnage.",
    },
    {
      titre: "Le Bras Mort — comment Le Banc a été déclassé",
      categorie: "Approfondissements par faction & peuple",
      contenu:
        "Personne n'a causé l'envasement. Quelqu'un a décidé où il se déposerait.\n\nIl y a une quarantaine d'années, les chantiers navals de Mornac s'agrandissent et la vallée de la Lisdane se déboise en amont pour leur fournir du bois. Des versants nus saignent du limon à chaque crue, et le fleuve charrie désormais ce qu'il ne charriait pas.\n\nEn aval, à Libris, la même expansion produit l'effet inverse : les coques venues de Mornac sont plus grandes, le commerce de toile de sève s'intensifie et réclame des quais plus longs, et le bras est n'offre plus assez de tirant d'eau à marée basse. Le Comptoir fait voter la Digue Neuve — une estacade qui resserre le courant dans le bras est pour l'approfondir par chasse naturelle — et allonge ses quais dans la foulée.\n\nLes deux décisions sont raisonnables. Prises séparément, à deux cents lieues l'une de l'autre, par des gens qui ne se sont jamais parlé. Ensemble, elles tuent le bras ouest : le limon arrive, et le seul bras sans courant pour l'évacuer est celui que la digue vient de priver.\n\nEn une décennie, les quais de la Rangée B deviennent inaccessibles à marée basse, puis tout court. Il y a une trentaine d'années, le Comptoir déclasse la rangée entière et rachète les parcelles au prix déclassé : techniquement une indemnisation, assez pour qu'on ne puisse pas attaquer, pas assez pour survivre. Les familles ruinées se replient sur Le Banc, la seule terre encore abordable, et qui donne sur le bras mort. Elles s'y réveillent depuis trente ans face aux quais qui les ont ruinées.\n\nCE QUI REND LE GRIEF INÉPUISABLE\n\nIl y a une faute, elle est réelle, et elle est partielle. Le Comptoir n'a pas fait le limon — il a seulement choisi quel côté de la ville s'en chargerait. On peut en discuter trente ans sans conclure, et c'est exactement ce qu'on fait sur Le Banc.\n\nLe Banc est humain parce que le Tissage n'a jamais été sur la Rangée B : le Cercle des Peuples a sa propre économie et s'occupe des siens. Quand le bras ouest est mort, les ruinés étaient presque tous humains, et le Tissage n'a rien senti passer. Il est faux que le Tissage y soit pour quelque chose. Il est parfaitement vrai qu'il n'en a pas souffert — et c'est tout ce dont les Fils de Libris ont besoin.\n\nPourquoi la colère ne vise pas le Comptoir, qui l'a réellement provoquée ? Parce qu'on ne poursuit pas quelqu'un qui vous a indemnisé, et parce que le Comptoir tient encore l'argent, le port et le budget de l'Assemblée. Le Tissage, lui, n'a ni troupes ni trésor, et il est visible.\n\nDeux noms pour la même chose : la rue dit l'Envasement, le registre dit le Reclassement de la Rangée B. Qui emploie le second n'a jamais mis les pieds sur l'île.\n\nDernière plaie, la plus fraîche : le Comptoir n'a jamais dragué pour sauver la Rangée B, mais il drague aujourd'hui pour vendre le limon aux Aelindra. Les marchands du Banc regardent passer sous leurs fenêtres les chalands chargés de la boue qui les a ruinés.",
    },
    {
    ...
    {
      titre: "Les Brisants — la ruée",
      categorie: "Approfondissements par faction & peuple",
      contenu:
        "On y extrait un minerai ordinaire — rien de rare, rien de magique. Sa seule vertu est d'être abondant et sans propriétaire, et c'est précisément ce qui rend le site dangereux : aucune raison d'être prudent, aucune raison d'être discret, aucune autorité à qui en demander la permission. Une ruée sur une plaie.\n\nMornac et le Comptoir de Libris y exploitent en concurrence ouverte. Pas de compagnie mixte, pas de régulation, pas de responsabilité identifiable — le jour où quelque chose tournera mal, chacun accusera l'autre avant de regarder ce qui s'est réellement passé.\n\nMornac s'y comporte comme il s'est déjà comporté à l'intérieur des terres : extractif, à courte vue, sans se soucier de ce que ses choix coûteront ailleurs — le même trait qui a fini par envaser le bras ouest de la Lisdane et ruiner la Rangée B (cf. « Le Bras Mort — comment Le Banc a été déclassé »). Aux Brisants, il n'y a personne en aval pour en payer le prix à sa place — ou du moins, personne qui le sache encore.\n\nCe n'est pourtant pas le minerai qui rend la ruée fiévreuse. Les Brisants poussent aussi la Pâlefeuille, une plante qu'on ne trouve nulle part ailleurs — et Mornac comme le Comptoir se l'arrachent en plus du minerai. Le minerai est banal et lourd ; la Pâlefeuille est légère, chère, et se revend au détail dans toutes les tavernes de Libris. C'est elle qui paie les expéditions qui ne rapportent rien, et elle qui attire ceux que le minerai seul n'aurait jamais tentés.",
    },
    {
      titre: "Les Sylvareth — les elfes de l'archipel",
      categorie: "Approfondissements par faction & peuple",
      mjSeulement: true,
      contenu:
        "Quatrième nation elfique. Servants des trois visages qui ont perdu la Guerre du Trône. Ils ne se voient pas comme des renégats mais comme le clergé légitime d'un camp spolié.\n\nCe sont les déicides. Ce sont leurs ancêtres qui ont accompli le rituel contre Ysmaal. Aucune défaite militaire ne justifierait leur bannissement : celui-là, si.\n\nL'archipel est le cratère du déicide, au nord-est du continent occidental — entre les deux continents, donc idéalement placé pour raider les côtes elfiques et s'asseoir sur les routes maritimes qu'Aetharion emprunte vers les Marches Orientales.\n\nLe piège. Coupés de l'Arbre, leurs âmes tombent dans la Mer des âmes — où leurs propres dieux les dévorent. Ils ne peuvent ni prier, ni se repentir, ni changer de camp. Ce ne sont pas des fanatiques : ce sont des otages qui font un pas de plus dans le crime pour survivre au précédent.\n\nLeur projet : réinstaurer un dieu de la Mort — remettre un berger sur le troupeau — et rentrer à l'est.\n\nApparence majoritairement intacte, mais les instincts convergent vers ceux des orques. Or les orques sont les corps des Consciences de Violence et de Meurtre : les Sylvareth ne « deviennent pas mauvais », ils sont colonisés par les consciences que leurs propres dieux cultivent. Ils le nieraient avec la dernière énergie. Même motif que les Khazrak Dûm, transposé chez les elfes.\n\nTrois courants, qui ne s'opposent pas sur la morale du déicide mais sur à qui la faute si on nous mange :\n• Les fiers — acte fondateur assumé.\n• Les honteux — trop orgueilleux pour reconnaître l'erreur.\n• Les accusateurs — se croient boucs émissaires d'un crime des Gardiens.\n\nRaids indiscriminés — tout ce qui flotte. Conséquence majeure : le monde ne voit pas une théologie, il voit un problème de piraterie. Un facteur du Comptoir qui perd une cargaison n'imagine pas que ses assaillants tentent de ressusciter un dieu ; il augmente sa prime d'assurance. Personne n'a de raison de creuser, et c'est pourquoi leur nature reste incertaine partout. Le Comptoir a un dossier « pertes maritimes » qui traîne depuis des générations et que nul n'a jamais lu autrement que comme une ligne comptable.\n\nConnus à l'est à cause des raids, mais leur nature exacte reste indéterminée.\n\nIls tiennent les îles-mères du nord-est de l'archipel, autour des ruines d'Aethanor, l'ancienne capitale elfique — celle que les érudits des Académies de Liberra connaissent de nom sans jamais l'avoir vue. Eux seuls nomment l'archipel entier : Ysmareth, du nom d'Ysmaal et de la terminaison -eth des déchus. Aucun humain n'a jamais nommé l'ensemble — chacun ne voit qu'un morceau.\n\nLeurs raids indiscriminés prennent un relief neuf depuis que Mornac et le Comptoir prospectent les Brisants, aux franges occidentales du même arc (cf. « Les Brisants — la ruée ») : les navires de prospection croisent désormais dans des eaux que les Sylvareth considèrent comme les leurs. Ni les prospecteurs ni les Sylvareth ne comprennent ce que fait l'autre — les uns y voient un gisement, les autres une intrusion sur leur périmètre de chasse.",
    },
    {
      titre: "Notes MJ — l'archipel qu'on ne voit pas",
      categorie: "Notes MJ",
      mjSeulement: true,
      contenu:
        "Le point central, à ne jamais faire dire par un PNJ\n\nAucun humain ne sait qu'il s'agit d'un seul archipel. Solvarn voit les Ruines Blanches au large et n'y touche pas ; Mornac et Liberra voient les Brisants et y voient un gisement. Personne n'a tracé la ligne entre les deux — et c'est précisément découvrable en jeu : un cartographe, un relevé de sondes, un érudit des Académies superposant deux portulans suffisent. Aucun PNJ n'a besoin de l'expliquer aux joueurs ; c'est à eux de le trouver.\n\nL'évitement solvarien\n\nL'Église a sous les yeux, au large de ses propres côtes, des ruines manifestement elfiques, pendant qu'elle enseigne que les dieux elfiques sont des démons. Elle n'a donc aucun intérêt à y envoyer une expédition. Le désintérêt impérial pour ces îles, malgré des siècles de puissance navale, n'est pas de la négligence : c'est de l'évitement.\n\nLe piège mécanique — un site minier permanent emploie des mages\n\nVentilation, éclairage, levage : l'exploitation des Brisants tourne sur de la magie de source Mer, en continu, sur une membrane déjà déchirée (seuils de Déchirure à 12/24/36/48 PP, cf. « La membrane et les Remous »). Quelques semaines d'exploitation suffisent à y creuser une Plaie. Hameçon prêt à l'emploi : le Comptoir a un site qui ne répond plus.\n\nLe piège stratégique — personne à l'Assemblée n'a fait le calcul\n\nLiberra et Mornac fabriquent des démons à l'ouest, aux Brisants, pendant que le seul contre-poison connu — le Grisfer — reste un monopole de Kaldrun, alliée de l'Empire. Plus la ruée s'étend, plus la République dépend militairement d'un fournisseur qui relève du camp d'en face. Personne, à l'Assemblée de Libris, n'a encore posé les deux faits côte à côte.",
    },
    {
...
const CHRONOLOGIE = [
  {
    ...
    description:
      "Des siècles de conflit divin entre Gardiens de l'Écorce et Enfants de la Sève, jusqu'au déicide d'Ysmaal, quatrième Gardien et visage de la Mort : un rituel censé défaire son concept en fait mourir le visage. Le contrecoup fracture littéralement le continent occidental. Le sang versé pendant la Fracture donne naissance aux Humains ; les Enfants de la Sève vaincus sont bannis — non exécutés — dans la Mer des âmes, où ils deviennent les Dieux du Chaos ; le continent occidental est ravagé, les Elfes survivants se replient à l'est.",
  },
  {
    id: "deicide-ysmaal",
    periode: "Le déicide d'Ysmaal",
    quand: "il y a ~2000 ans",
    duree: null,
    peuples: ["elfes", "chaos"],
    description:
      "Poussés par les Enfants de la Sève, des servants elfes accomplissent un rituel destiné à défaire le concept de Mort porté par Ysmaal, quatrième Gardien, seul concept majeur jamais apparié du panthéon. Mais un dieu est son concept : atteindre l'un tue l'autre. Le rituel fracture littéralement le continent occidental et sépare l'ancienne capitale elfique — Aethanor — du reste du monde : naissance, au nord-est, de l'archipel que les elfes nomment Ysmareth, du nom d'Ysmaal et de la marque des déchus. Les elfes qui l'ont accompli, exilés avec les Enfants bannis, deviennent les Sylvareth.",
  },
  {
    id: "chute-nyaleth",
    periode: "La chute de Nyaleth",
    quand: "un peu après la Fracture",
    duree: null,
    peuples: ["chaos"],
    description:
      "Visage déchu de la Vie (Maladie & Putréfaction), Nyaleth tombe à son tour — mais trop tard : le panthéon, épuisé par la Guerre du Trône et le déicide d'Ysmaal, n'a plus la force de le bannir comme les Enfants de la Sève. Il reste libre et ancré dans le monde, seul dieu tombé qu'on puisse encore rencontrer, atteindre, marchander ou tuer.",
  },
  {
    id: "ere-tribus",
  ...
  {
    id: "decouverte-brisants",
    periode: "Premières prospections aux Brisants",
    quand: "il y a ~5 ans",
    duree: null,
    peuples: ["humains"],
    description:
      "Des sondeurs mornacois, puis des équipes du Comptoir de Libris, repèrent un gisement de minerai ordinaire sur les franges rocheuses au large des côtes occidentales — les Brisants. Rien de rare, rien de magique : sa seule vertu est d'être abondant et sans propriétaire. Aucune compagnie mixte, aucune régulation, aucune autorité à qui en demander la permission. Mornac et le Comptoir s'y installent en concurrence ouverte, chacun creusant sans regarder ce que fait l'autre.",
  },
  {
...
const PNJ_CLES = [
  ...
  },
  {
    ...
    rencontre: true,
    resume:
      "Le clerc qui avait annoncé la pénurie au marché lors de la distribution. Roué de coups en menant un recensement de nuit, puis achevé quatre nuits plus tard dans son lit.",
    description:
      "Consciencieux jusqu'à l'imprudence, Denis recensait les maisons du Tissage vers deux heures du matin, seul, une lanterne dans une main et son carnet dans l'autre — la seule heure où tout le monde est chez soi, et exactement ce qui le rendait insupportable. À la quatrième maison d'une même nuit, on l'a remis à la rue ; vingt pas plus loin, on l'a roué de coups sans rien lui voler. Jambe et côtes brisées, la tête ouverte contre un poteau. Stabilisé par Mira Sylvenne, transporté chez un guérisseur près du Palais du Serment, il allait s'en remettre.\n\nQuatre nuits plus tard, quelqu'un est entré au changement de veille et l'a achevé d'un seul coup porté de l'autre côté du crâne. Le guérisseur a écrit qu'il avait succombé à ses blessures et l'a cru sincèrement. Il faisait correctement son travail, et c'est tout ce qu'on pourra jamais lui reprocher.",
    accroches: [
      "MJ — ses registres de recensement ne contiennent rien sur les patrouilles ni sur Aldenne — seulement des foyers, des âges, des besoins, et deux mentions « à vérifier » en marge. Ne pas en faire un indice supplémentaire sur l'entrepôt n° 7, c'est un fil séparé.",
      "MJ — son agression et son meurtre n'ont aucun commanditaire commun, et aucun commanditaire tout court. Ne jamais en fabriquer un pour rendre l'enquête plus satisfaisante : si la table en exige un, la laisser faire, ne rien confirmer, et laisser un fait la démentir plus tard.",
      "MJ — aucun habitant du Tissage ne l'a frappé. Ce que le quartier protège en se taisant n'est pas un coupable : c'est une porte fermée en pleine nuit, qui n'est un crime pour personne et qui, dite à voix haute dans cette ville, devient « le Tissage l'a jeté dehors et il est mort ».",
    ],
  },
  {
    id: "mira-sylvenne",
  ...
  },
  {
    id: "aude-berlan",
    nom: "Aude Berlan",
    titre: "Professeure à l'Atelier des Marges",
    faction: "Citoyens de Libris",
    rencontre: true,
    resume:
      "Enseigne à l'académie dissidente de Libris depuis assez longtemps pour avoir formé une génération. Ancienne professeure de Kylhian Bondimir, et sœur d'Aubin Sorre.",
    description:
      "Née Sorre. Elle reçoit dans son bureau plutôt que dans une salle officielle, faute de place ailleurs, avec une théière tiède qu'elle ne fait jamais vraiment chauffer. Elle constate plus qu'elle ne s'émeut, ne s'excuse pas volontiers, et ne remercie pas beaucoup : ce n'est pas son registre. Elle a épousé Frédérik Berlan il y a plus de trente ans et n'a jamais quitté Libris.\n\nTrois de ses étudiants — Colin Vasarel, Iris Fontenay et Tomas Guérin — ont acheté au marché noir une relique démoniaque avec l'argent de l'Atelier et l'ont sondée jusqu'à ce qu'elle cède, dans une réserve du bâtiment. Ils sont vivants parce que le groupe est arrivé à temps. L'Atelier a perdu un Voile de sève acheté pour protéger ses serres, et gagné une rumeur dont il ne se remettra pas vite.",
    accroches: [
      "Son frère Aubin est mort il y a dix ans sur son site de recherche, et personne n'est allé chercher ses papiers — elle la première. Elle porte cette honte sans en parler, et c'est ce qui rend la remise de son cahier de relevés autrement plus lourde qu'une remise de documents.",
      "Elle veut finir le travail d'Aubin sur une plante qui coupe la faim : c'est long, ça se compte en saisons, et ça demande des lampes runiques naines — donc de traiter avec Kaldrun ou avec Serval. C'est l'accroche de fond la plus lente et la plus utile de la campagne.",
      "Elle demande au groupe que les noms de ses trois étudiants ne sortent pas, et portera la faute elle-même s'il en faut une. Accepter lui ouvre l'Atelier en entier — herbier, serres, bureau ; livrer les noms ne la fâche pas, la ferme définitivement.",
      "MJ — elle ignore totalement ce que la toile de sève fait à la membrane. Le Voile avait été acheté « pour protéger la serre de ce que le Chaos dépose ». Personne à Libris ne sait, et c'est un fait de lore à protéger : la découverte serait d'échelle politique, pas de détail de scène.",
      "MJ — elle n'a aucun lien avec l'Inquisition, le Comptoir ou le Roi des Mendiants, et rien à cacher. Elle ignore ce que son frère avait réellement trouvé, et ce que d'autres en ont fait après sa mort.",
    ],
  },
  {
    id: "frederik-berlan",
    ...
    accroches: [
      ...
      "MJ — aucun secret, aucune double vie, aucune dette. Sa fonction est d'être exactement ce qu'il paraît, dans une campagne où presque personne ne l'est.",
    ],
  },
  {
    id: "aubin-sorre",
    nom: "Aubin Sorre",
    titre: "Botaniste de l'Atelier des Marges (mort)",
    faction: "Citoyens de Libris",
    resume:
      "Frère d'Aude Berlan. A passé vingt ans à chercher comment cultiver une plante qui coupe la faim. Mort il y a dix ans sur son site de recherche.",
    description:
      "Botaniste, donc mineur aux yeux de l'Académie Libre, et parfaitement à sa place à l'Atelier des Marges. Il travaillait seul, loin, des saisons entières sans donner de nouvelles, sur une racine qui noircit au soleil et qu'il fallait apprendre à cultiver sous lampes runiques naines. Il avait résolu la culture ; il lui restait deux ou trois essais.\n\nIl est mort sur place, et personne de l'Atelier n'est allé chercher ses papiers — pas même sa sœur. Son cahier de relevés est resté dix ans là où il l'avait laissé, avant que le groupe ne le rapporte.",
    accroches: [
      "Son cahier est le point de départ d'un travail que quelqu'un peut reprendre. Une plante qui coupe la faim, dans une ville qui compte ses réserves, n'est pas une curiosité de botaniste.",
      "MJ — il est mort sans testament, sans héritier désigné et sans que personne ne réclame rien. Dix ans pendant lesquels ses travaux étaient à prendre, pour qui savait où ils étaient.",
    ],
  },
  {
    id: "marec-vannier",
    nom: "Marec Vannier",
    titre: "Ancien peseur-juré de la Rangée B",
    faction: "Citoyens de Libris",
    resume:
      "Soixante-trois ans. Portefaix à la journée sur les quais depuis trente ans, après avoir certifié les balances du Comptoir pendant vingt.",
    description:
      "Aide à quinze ans, juré à vingt-cinq : c'est lui qui disait si une balance mentait, et les deux côtés le croyaient. L'envasement lui a pris sa rangée, sa charge et sa licence d'un seul coup — il avait trente-trois ans. Il a gardé son bandeau de portefaix, sa plaquette de laiton et sa boîte de poids étalons, et il n'a plus rien pesé depuis.\n\nIl vit au Banc, dans la même rue que les familles ruinées en même temps que la sienne. On le connaît, on le salue, personne ne lui demande rien. Il ne ment jamais et ne se plaint jamais.",
    accroches: [
      "Un homme qui a passé sa vie à garantir que les mesures étaient justes, et qui a vu s'en servir pour reprendre une rue entière.",
      "Il a déposé une requête écrite sur le déclassement, dix-neuf ans après les faits. Il a reçu une réponse imprimée, la même que tout le monde, et il l'a gardée.",
    ],
  },
  {
    id: "nelis",
    nom: "Nélis",
    titre: "Apprentie tisserande au Tissage",
    faction: "Cercle des Peuples",
    resume:
      "Dix-neuf ans, Aelindra. Du quartier sans y être inscrite : elle loge chez sa maîtresse, sur une parcelle où elle ne figure pas.",
    description:
      "Apprentie sans titre, logée et nourrie contre son travail, dans un arrangement que tout le monde au Tissage trouve normal et qu'aucun registre du Comptoir ne prévoit. Une bouche de plus que le rôle de parcelle, et rien pour l'expliquer sur le papier.\n\nSi le Comptoir s'en aperçoit, sa maîtresse risque l'amende et elle son apprentissage. C'est tout ce qu'elle a.",
    accroches: [
      "Ce qui la met en danger n'est pas un crime, c'est une irrégularité administrative — et c'est exactement pour ça qu'elle est sans recours.",
      "Sa maîtresse ne parlera à personne, dans aucune circonstance. Nélis parlera à qui lui apporte une garantie tenable, et à personne d'autre.",
    ],
  },
  {
    id: "piq",
    nom: "Piq",
    titre: "Enfant du Banc",
    faction: "Citoyens de Libris",
    resume:
      "Quinze ans, demi-gobelin. Un des Doigts. Vend ce qu'il a vu contre un repas, jamais contre des pièces.",
    description:
      "Né au Banc, sur l'île des déclassés, et pas plus attaché à un toit qu'à un autre. Il passe par les toits végétalisés plutôt que par les rues, suit volontiers les gens qui font quelque chose d'inhabituel, et se souvient très bien de ce qu'il regarde.\n\nIl connaît un intermédiaire, jamais deux, et ne sait pas pour qui il travaille. Ça ne lui paraît pas étrange : on le nourrit, il rend des services.",
    accroches: [
      "Payé en pièces, il les prend et devient méfiant. Nourri, il revient — et il revient longtemps.",
      "MJ — premier contact réel avec la méthode du Roi des Mendiants, et il ne doit rien révéler : pas de nom, pas de réseau, pas de révélation. Un gamin payé en nourriture qui ne trouve pas ça étrange. Ne jamais lui donner de sobriquet ; si la table lui en trouve un, l'adopter aussitôt.",
    ],
  },
  {
    id: "corin-vashtel",
  ...
  },
  {
    ...
    faction: "Fils de Libris",
    resume: "Ruiné par la concurrence des guildes du Comptoir, pas violent, mais assiste aux discours de la Racine Dorée faute d'alternative.",
    description:
  ...
  },
  {
    ...
    accroches: [
      ...
      "MJ — troisième piste, la moins gardée : la taverne qu'Ewen désignait s'appelait alors La Racine Noire, et elle a été rachetée puis rebaptisée La Racine Dorée peu après la période qu'il décrivait. Une enseigne, ça se date — n'importe quel habitué du Banc peut dire à quel mois la dorure est apparue. C'est la seule des trois voies vers le financement solvarien qui ne demande à personne de trahir sa hiérarchie.",
    ],
  },
  {
    id: "ysoline-marnet",
    nom: "Ysoline Marnet",
    titre: "Hydrographe du Comptoir, à la retraite",
    faction: "Citoyens de Libris",
    resume: "Soixante-quinze ans, vit sur Le Banc, et savait il y a quarante ans exactement ce qui allait s'y passer.",
    description:
      "Elle avait la trentaine et aucun nom quand elle a déposé, devant l'Assemblée, un mémoire expliquant que la Digue Neuve déplacerait le dépôt de limon sans le supprimer — et que le bras ouest, privé de courant, s'ensablerait en une décennie. Son raisonnement demandait d'admettre que la prospérité des chantiers de Mornac coûterait quelque chose à Libris. On l'a écoutée poliment, et on a voté.\n\nElle vit aujourd'hui sur Le Banc, au milieu de gens qu'elle a essayé de sauver et qui ne le savent pas. C'est la seule habitante de l'île qui n'accuse pas le Quartier du Tissage, parce qu'elle sait précisément ce qui s'est passé — ce qui l'y rend aussi seule qu'ailleurs.",
    accroches: [
      "Elle peut tout expliquer, calmement, avec des chiffres. Personne ne l'écoute depuis quarante ans, et elle a cessé d'insister — il faut venir la chercher.",
      "Son mémoire original dort aux archives du Palais du Serment. Aldous Kenrick a remarqué que d'anciens dossiers ont été consultés récemment sans savoir par qui.",
      "MJ — c'est ce dossier-là. Quelqu'un reconstruit la preuve que le Comptoir a ruiné Le Banc, et le seul bloc qui gagne à l'établir est celui qui y tient sa taverne. Dessalles aurait pour la première fois un grief vérifiable — et il désignera quand même le Tissage plutôt que le Comptoir, parce que le Comptoir peut riposter.",
    ],
  },
  {
    id: "bob-b148",
    ...
    accroches: [
      ...
      "MJ — son fournisseur figure au registre des Fondations, et les personnages y ont reconnu son nom. Il ne livre plus parce qu'il est passé de l'autre côté : il ne vend plus ce qu'il reçoit. Le dire à Bob est gratuit et coûte tout — il ne se fâchera pas, il ne dénoncera personne, et il regardera sa propre cloison murée en se demandant depuis quand. C'est la scène la plus lourde disponible dans le quartier et elle ne demande aucun jet.",
    ],
  },
  {
    id: "henry-b148",
  ...
  {
    id: "ombeline-fondations",
    nom: "Ombeline",
    titre: "Doyenne des celliers de la Rangée B",
    faction: "Citoyens de Libris",
    rencontre: true,
    resume:
      "Quatre-vingts ans, née sur la Rangée B avant l'Envasement — restée sous terre parce qu'elle ne marche plus assez vite, et qu'elle le sait.",
    description:
      "Elle a vécu sur la Rangée B du temps où les quais servaient encore, et elle vit aujourd'hui sous elle, dans les celliers à grain que le Comptoir a fait murer au déclassement. Une trentaine de personnes y logeaient avec elle. Quand des étrangers sont descendus, tout le monde est parti sans panique ni désordre ; elle est restée, faute de pouvoir suivre.\n\nElle n'a peur de personne — à son âge, dit-elle, on a fini d'avoir peur des gens armés — et elle répond à tout ce qu'on lui demande, ce qui est exactement ce qui rend la conversation difficile. Le grain vient du Comptoir, c'est marqué dessus. Des porteurs l'ont descendu, c'est leur métier. Personne n'a rien demandé à personne : il y a des gens qui ont faim et du grain qui dort. Un monsieur passe et tient le registre ; il n'a pas donné son nom, et on ne demande pas le nom de quelqu'un qui vous apporte à manger.",
    accroches: [
      "MJ — elle dit toute la vérité qu'elle connaît, et elle ne connaît aucun nom. Aucun jet ne trouvera de mensonge parce qu'il n'y en a pas. Si on insiste, elle répète la même chose plus lentement : le mur n'est pas un secret bien gardé, c'est qu'il n'y a personne à trahir.",
      "MJ — les personnages ont laissé le grain et prévenu la Garde. Elle ne leur reprochera ni l'un ni l'autre, et ne fera aucun lien entre les deux. C'est aux joueurs de le faire, plus tard et seuls : laisser les sacs était de la pitié, prévenir était du devoir, et c'est la somme des deux qui vide les celliers. Ne jamais le formuler à leur place.",
      "MJ — la tuer ou la faire disparaître serait le levier le moins cher de tout l'arc. Ne pas l'employer : sa valeur dramatique tient entièrement à ce qu'elle reste disponible et inutile.",
    ],
  },
  {
    id: "perrine-sauvel",
    nom: "Perrine Sauvel",
    titre: "Tenancière de La Racine Dorée",
    faction: "Citoyens de Libris",
    resume:
      "Tenait déjà le comptoir quand l'enseigne s'appelait La Racine Noire — rachetée avec les murs, et restée.",
    description:
      "Le mouvement a racheté la taverne, pas la femme qui la tient. Perrine servait déjà derrière ce comptoir bien avant que l'enseigne ne soit redorée, et elle a continué après sans qu'on lui demande son avis sur le changement de nom. Elle sert les Fils de Libris comme elle servait les bateliers avant eux : vite, sans commentaire, et sans jamais s'asseoir.\n\nSon père avait une parcelle sur la Rangée B. Elle avait dix ans à l'Envasement et se souvient surtout de l'hiver qui a suivi.",
    accroches: [
      "Elle peut dater la redorure de l'enseigne au mois près — elle était derrière le comptoir ce jour-là. Elle le dira sans y voir malice à quiconque le lui demande, ce qui en fait la voie la plus courte vers la troisième piste d'Ewen Castellan.",
      "MJ — sa dette date de cet hiver-là : quelqu'un a nourri la famille Sauvel, et personne n'a jamais rien réclamé. Cette année, un homme en habits sombres est passé lui demander de ne rien faire : laisser certaines conversations se tenir à son comptoir, et répéter deux ou trois choses aux bonnes oreilles. Elle ne considère pas cela comme un service rendu.",
      "MJ — deux créanciers dans la même salle, qui s'ignorent : l'argent solvarien a acheté les murs, le Roi des Mendiants tient la femme qui les tient. Dessalles ne sait ni l'un ni l'autre. Ne jamais les faire converger dans une même scène — leur indépendance est ce qui rend la taverne illisible.",
    ],
  },
  {
    id: "ilsevar-cendreau",
  ...
  {
    id: "yselde-maren",
    nom: "Comtesse Yselde Maren",
    titre: "Comtesse de Mornac",
    faction: "Mornac",
    secret: false,
    resume: "Dynastie marchande devenue dynastie régnante — gouverne Mornac avec un pragmatisme que la chevalerie de façade ne dissimule plus vraiment.",
    description:
      "Héritière d'une lignée de familles marchandes qui a fini, deux générations après avoir financé la survie du Siège de Mornhaven, par convertir ce levier économique en couronne elle-même. Yselde gouverne comme elle a appris à commercer : sans sentiment inutile, chaque alliance et chaque concession évaluées à leur juste prix. Son mariage avec Bastien Rovelle, issu d'une grande famille d'armateurs, a consolidé deux fortunes marchandes en une seule couronne — une union qu'elle ne renie pas, même si elle laisse volontiers à son époux la gestion de la flotte pour se concentrer sur l'État.\n\nUtilise Corvina Aldren comme instrument discret pour déstabiliser la guilde marchande grandissante de la République de Liberra — une politique qu'elle assume en privé sans jamais l'admettre publiquement. Ses deux enfants, Basile et Cassia, incarnent deux futurs possibles pour Mornac : la continuité froide de la doctrine pragmatique pour l'un, une quête de légitimité plus personnelle pour l'autre — une divergence que Yselde observe avec une inquiétude qu'elle ne montre jamais.",
    accroches: [
      "Yselde pourrait solliciter des PJ étrangers pour une opération qu'elle ne peut confier à ses propres agents sans risquer un scandale politique.",
      "Le désaccord grandissant entre Basile et Cassia sur la conduite de Mornac pourrait la forcer à trancher publiquement plus tôt qu'elle ne le voudrait.",
      "Si l'opération de Corvina contre le Comptoir de Liberra est découverte, Yselde devra choisir entre la désavouer publiquement ou assumer le risque diplomatique.",
    ],
  },
  {
    id: "bastien-rovelle",
    nom: "Bastien Rovelle",
    titre: "Comte consort de Mornac",
    faction: "Mornac",
    secret: false,
    resume: "Marié à la Comtesse Yselde Maren — reste délibérément à l'écart de la politique du Grand Livre, gérant directement la flotte marchande familiale.",
    description:
      "Issu de la famille Rovelle, l'une des grandes lignées d'armateurs de Mornhaven, Bastien a vu son mariage avec Yselde consolider deux fortunes marchandes en une seule couronne — une union aussi sentimentale que commerciale, ce que ni l'un ni l'autre ne renie. Il laisse à son épouse la conduite de l'État et se consacre à la flotte, qu'il connaît mieux que quiconque à la cour.\n\nSes enfants, Basile et Cassia, le voient comme le parent le plus accessible — Cassia en particulier, qui partage son amour de la mer bien plus que l'intérêt de sa mère pour le Grand Livre.",
    accroches: [
      "Bastien pourrait devenir un intermédiaire discret entre Cassia et sa mère lorsque leurs désaccords sur les méthodes de Corvina Aldren s'enveniment.",
      "Un problème touchant la flotte marchande (piraterie, sabotage) serait naturellement son terrain, plutôt que celui de Yselde.",
      "Sa neutralité politique pourrait être testée si un scandale forçait la cour à choisir un camp entre ses deux enfants.",
    ],
  },
  {
    id: "basile-maren",
    nom: "Basile Maren",
    titre: "Héritier de Mornac (26 ans)",
    faction: "Mornac",
    secret: false,
    resume: "Embrasse la doctrine pragmatique de sa mère avec une froideur qui dépasse encore la sienne.",
    description:
      "Basile n'a jamais remis en question la philosophie marchande de Mornac — il l'a simplement poussée plus loin que sa mère n'a jamais osé, sans les scrupules résiduels qu'elle garde encore en privé. Prépare sa succession en resserrant les liens du Grand Livre plutôt qu'en les questionnant, et voit dans les méthodes de Corvina Aldren contre Liberra une prudence excessive plutôt qu'un risque à minimiser.\n\nSa relation avec sa sœur Cassia s'est distendue avec les années : là où elle rêve de légitimité gagnée en mer, il ne voit qu'une romantique dérive loin des vrais leviers du pouvoir — le registre, pas le large.",
    accroches: [
      "Basile pourrait pousser Mornac vers une politique plus agressive envers le Comptoir de Liberra que ne le souhaite sa mère.",
      "Un conflit de succession larvé avec Cassia pourrait éclater si Yselde venait à favoriser publiquement l'une des deux visions.",
      "Sa froideur calculée pourrait en faire une cible ou un allié de choix pour quiconque cherche à influencer durablement Mornac.",
    ],
  },
  {
    id: "cassia-maren",
    nom: "Cassia Maren",
    titre: "Cadette de Mornac (22 ans)",
    faction: "Mornac",
    secret: false,
    resume: "Rêve d'une légitimité gagnée en mer plutôt qu'achetée au registre — bien plus proche de son père que de sa mère.",
    description:
      "Contrairement à son frère Basile, Cassia ne trouve aucun honneur dans les manœuvres du Grand Livre — elle veut prouver sa valeur sur l'eau, comme son père Bastien avant elle, plutôt que par le mariage ou l'héritage. Passe autant de temps que possible sur la flotte marchande familiale, au grand dam d'une mère qui y voit une distraction plutôt qu'une préparation au pouvoir.\n\nSes désaccords avec Corvina Aldren sur les méthodes employées contre la guilde de Liberra sont les seuls qu'elle ose exprimer ouvertement à la cour — une prudence calculée qui cache une opposition plus large à la doctrine froide de sa mère et de son frère.",
    accroches: [
      "Cassia pourrait être la première à découvrir un indice compromettant sur les opérations de Corvina Aldren, sans savoir quoi en faire.",
      "Son père Bastien pourrait devenir son intermédiaire discret lorsque ses désaccords avec sa mère s'enveniment.",
      "Un problème en mer (piraterie, sabotage) pourrait lui donner l'occasion de prouver sa valeur loin de la cour, aux côtés de PJ.",
    ],
  },
  {
    id: "corvina-aldren",
  ...
  {
    id: "astrid-sten",
    nom: "Astrid Sten, née Hollenbeck",
    titre: "Margravine consort de Serval",
    faction: "Serval",
    secret: false,
    resume: "A grandi en partie parmi les Nains de l'Ordre — incarne la philosophie d'alliance de Serval plus profondément encore que Torvald lui-même.",
    description:
      "Fille d'une famille servaline détentrice de droits miniers près de Kaldrun, Astrid a passé une partie de son enfance parmi les Nains de l'Ordre avant son mariage avec Torvald. C'est elle, dit-on à la cour, qui a le plus soufflé l'idée d'approfondir le commerce avec l'enclave d'Aelindra — une initiative que leur fils Halvard a reprise à son compte avec un enthousiasme qu'il croit entièrement personnel.",
    accroches: [
      "Astrid pourrait révéler discrètement à des PJ que l'ambition d'Halvard envers Aelindra vient en réalité d'elle, une information qui changerait la perception de sa position à la cour.",
      "Ses liens personnels avec Kaldrun pourraient en faire une intermédiaire de choix si une crise diplomatique éclatait avec l'Ordre.",
      "Elle pourrait chercher à apaiser le conflit naissant entre Halvard et Sigrun avant qu'il ne devienne une crise de succession ouverte.",
    ],
  },
  {
    id: "halvard-sten",
...
const FACTIONS = [
  ...
  },
  {
    ...
    // Carte de la capitale, rendue dans l'onglet Factions (rendreFactions).
    // Les sections de lore taguées deplaceVersFaction: "République de Liberra"
    // (Les Lieux de Libris, La Fièvre du Grenier) y sont rattachées.
    carte: "assets/ville/Libris.png",
    histoire:
    ...
    update:
      "LA FIÈVRE DU GRENIER — niveau 4 sur 5\n\nLes torches qui marquaient les rondes de la Garde dans le Tissage, plantées le long des rues depuis des mois, se sont éteintes une à une au fil des semaines. La dernière était celle de Denis Fabrelle, clerc du Comptoir, qui recensait les maisons du quartier de nuit, porte après porte, pour que le rationnement colle enfin à la réalité plutôt qu'à de vieux chiffres. Vers deux heures du matin, on l'a mis à terre à vingt pas d'une porte qui venait de se refermer sur lui : jambe et côtes brisées, la tête ouverte contre un poteau. Mira Sylvenne l'a stabilisé avant qu'on l'emmène chez un guérisseur près du Palais du Serment.\n\nQuatre nuits plus tard, quelqu'un est entré l'achever dans son lit. Le guérisseur a écrit qu'il avait succombé à ses blessures et l'a cru sincèrement. Un homme battu, c'est un fait divers ; un homme achevé, c'est une accusation qui cherche un coupable — et la ville en a trouvé un en deux jours, sans que personne l'ait proposé : on répète au Grand Marché qu'un clerc est allé compter les gens du Tissage et qu'on l'a tué pour ça.\n\nLa même semaine, on raconte qu'à l'Atelier des Marges, l'académie dissidente qui accueille les demi-races sans rien demander, quelque chose est sorti du sol un soir dans une réserve. Personne n'a de témoin, tout le monde a une certitude, et les deux histoires se recollent d'elles-mêmes dans les files du Grand Marché en une seule, la plus simple et la plus fausse.\n\nLe Comptoir a suspendu ses livraisons au Tissage « le temps que la sécurité soit rétablie » — une phrase que personne n'a signée et que tout le monde applique. Le quartier a mis des planches aux fenêtres, rangé les tréteaux de la place que Mira avait ouverte, et il ne s'arrête plus aux planches : on veille aux deux ponts, par tours, et on prend les noms de ceux qui s'inscrivent. Personne au Tissage n'appelle ça une milice. Personne au Grand Marché ne l'appelle autrement.\n\nLes prix montent parce que le stock disparaît des étals, et le stock disparaît parce que les prix montent. Il y a encore assez de grain dans la ville pour trois mois — personne n'y croit. La Garde Citoyenne, déjà en sous-effectif, tient les cinq ponts et laisse les rues aux quartiers.\n\nÉchelle : 0 rien d'anormal · 1 la ville stocke · 2 un commerçant refuse ouvertement de servir un non-humain · 3 une rixe fait un blessé grave · 4 le Tissage se barricade et le Comptoir suspend ses livraisons au quartier (actuel) · 5 l'émeute part seule, il n'y a plus personne à convaincre.\n\nLe compteur monte d'un cran à chaque incident public non résolu, et descend d'un cran quand les personnages règlent visiblement quelque chose.",
    intro:
...
const FAUNE_FLORE = {
  ...
  entrees: [
    ...
    { nom: "Edelweiss", type: "flore", peuple: "commun", ou: "Haute montagne — falaises, éboulis et cols exposés de toute chaîne, quel que soit le versant. Ne pousse qu'en altitude", rarete: "peu_commun",
      description: "Petite fleur blanche en étoile, feuilles et pétales couverts d'un fin duvet argenté qui la protège du froid et du vent. Poussée isolée, jamais en tapis : chaque pied trouvé est une petite victoire pour qui grimpe le chercher.",
      usage: "Propriétés reconnues contre les poisons : séchée et réduite en poudre, elle entre dans la composition de l'antidote (cf. loot.json, item antidote) et de plusieurs contrepoisons de fortune utilisés par les guides de montagne. Cueillie fraîche, mâchée en urgence, elle ralentit un poison en attendant un vrai remède — un geste de survie plutôt qu'un traitement fiable." },
