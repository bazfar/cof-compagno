...
clesFusion.forEach((ids, cle) => {
  const restants = ids.filter((id) => !(DOUBLONS_AUTORISES[id] && ids.includes(DOUBLONS_AUTORISES[id])));
  if (restants.length > 1) restants.forEach((id) => signalerArme(id, `doublon (nom, degats, portee, typedegats, elementaire, touches) identique à : ${restants.filter((x) => x !== id).join(", ")}.`));
});
ARMES_MONSTRES.forEach((a) => {
